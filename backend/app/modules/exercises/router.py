...
async def update_exercise_endpoint(exercise_id: int, data: ExerciseUpdate, user_id: CurrentUserIdDep, session: SessionDep):
    return await update_exercise(exercise_id, user_id, data, session)


@router.delete("/{exercise_id}", status_code=204)
async def delete_exercise_endpoint(exercise_id: int, user_id: CurrentUserIdDep, session: SessionDep):
...
async def add_test_case_endpoint(exercise_id: int, data: TestCaseCreate, user_id: CurrentUserIdDep, session: SessionDep):
    return await add_test_case(exercise_id, data, user_id, session)


@router.delete("/{exercise_id}/test-cases/{tc_id}", status_code=204)
async def delete_test_case_endpoint(exercise_id: int, tc_id: int, user_id: CurrentUserIdDep, session: SessionDep):
