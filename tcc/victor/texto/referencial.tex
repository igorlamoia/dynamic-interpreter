...
Este capítulo contém o referencial que embasa os principais aspectos abordados ao longo
do trabalho.
...
Para a fundamentação teórica acerca do funcionamento, das fases e da arquitetura dos tradutores de linguagem, esta subseção adota como base estrutural a obra clássica de \citeonline{aho1995}, amplamente reconhecida na literatura de ciência da computação como uma das principais referências para o projeto de compiladores. Essa base é complementada por \citeonline{cooper2012}, que abordam o compilador sob a perspectiva de engenharia, e por \citeonline{appel2002}, que enfatiza a implementação prática das fases de análise e síntese.
...
Toda a estruturação das fases da compilação e os conceitos detalhados nesta seção fundamentam-se nas obras clássicas da área, notadamente nos estudos de \citeonline{aho1995}, \citeonline{price2001}, \citeonline{cooper2012} e \citeonline{appel2002}.
...
Os conceitos e definições inerentes à fase de análise léxica, detalhados a seguir, fundamentam-se fortemente nas obras de \citeonline{cooper2012}, \citeonline{nystrom2021}, \citeonline{price2001} e \citeonline{appel2002}.
...
As definições e conceitos apresentados nesta seção baseiam-se nos fundamentos estabelecidos por \citeonline{aho1995}, \citeonline{cooper2012}, \citeonline{price2001}, \citeonline{nystrom2021} e \citeonline{appel2002}.
...
Esta seção apresenta os fundamentos da análise semântica e da verificação de tipos, consolidando os conceitos e definições propostos por \citeonline{aho1995}, \citeonline{cooper2012}, \citeonline{price2001}, \citeonline{nystrom2021} e \citeonline{sebesta2018}.

Uma vez que a estrutura gramatical do código é validada pela análise sintática, o processo de compilação evolui para a fase semântica. O objetivo central aqui é garantir que as construções do programa, embora sintaticamente corretas, possuam coerência lógica para a futura execução. Enquanto o \textit{parser} se limita a verificar se a "forma" do código respeita as regras da linguagem, o analisador semântico atua como um inspetor que examina o significado das combinações entre os elementos do programa. Um dos pilares desse estágio é a resolução de nomes (ou amarrações), procedimento no qual o compilador mapeia cada identificador à sua declaração correspondente, respeitando as regras de escopo \cite{aho1995,cooper2012,sebesta2018}.

A verificação de tipos constitui outro componente vital da análise semântica. Utilizando a árvore de derivação e as informações armazenadas na tabela de símbolos, o compilador checa se os operadores estão sendo aplicados a operandos compatíveis. Durante essa inspeção, o sistema infere os tipos das expressões e detecta interpretações de valores que seriam semanticamente inválidas. Caso a linguagem permita, o analisador também pode aplicar coerções, que são conversões implícitas de tipos (como transformar um inteiro em ponto flutuante) para viabilizar operações entre tipos distintos \cite{price2001,nystrom2021,sebesta2018}.
...
Os conceitos e as técnicas referentes à geração de código intermediário e à tradução dirigida pela sintaxe, detalhados nesta seção, fundamentam-se nas obras de \citeonline{cooper2012}, \citeonline{nystrom2021}, \citeonline{price2001} e \citeonline{appel2002}.
...
As discussões e definições apresentadas nesta seção fundamentam-se nas obras de \citeonline{aho1995}, \citeonline{cooper2012}, \citeonline{price2001} e \citeonline{muchnick1997}.
...
Os conceitos, desafios e estratégias referentes à fase de geração de código objeto e mapeamento de arquitetura, descritos nesta seção, fundamentam-se nas obras de \citeonline{aho1995}, \citeonline{cooper2012}, \citeonline{appel2002}, \citeonline{muchnick1997} e \citeonline{price2001}.
...
% -------------------------------------------------------------------------------------------------------------
\subsection{TypeScript como Linguagem de Implementação}

Além das fases conceituais do compilador, a escolha da linguagem de implementação influencia diretamente a organização, a testabilidade e a segurança das estruturas internas manipuladas durante a tradução. Neste trabalho, o núcleo do interpretador foi implementado em TypeScript, linguagem mantida pela Microsoft que adiciona tipos opcionais ao JavaScript e gera, como saída, código JavaScript compatível com diferentes ambientes de execução \cite{microsofttypescript2026}.

Essa característica é relevante para a construção de ferramentas de compilação em ambiente web, pois permite que o mesmo núcleo de análise seja compartilhado entre navegador e servidor. No contexto deste projeto, estruturas como \textit{tokens}, configurações léxicas, nós lógicos da gramática e instruções intermediárias podem ser modeladas com tipos explícitos durante o desenvolvimento, reduzindo inconsistências entre as fases do compilador e favorecendo a manutenção incremental do sistema.

Do ponto de vista teórico, essa decisão dialoga com a importância atribuída aos sistemas de tipos na detecção antecipada de erros e na documentação estrutural dos programas. Embora a tipagem do TypeScript seja apagada na execução JavaScript, seu uso em tempo de desenvolvimento contribui para a validação estática das interfaces internas do compilador, aproximando a implementação prática dos princípios de verificação de tipos discutidos por \citeonline{aho1995}, \citeonline{cooper2012} e \citeonline{sebesta2018}.
