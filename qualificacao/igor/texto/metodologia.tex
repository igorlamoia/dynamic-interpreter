...
Quanto à divisão de trabalho, não houve especialização de papéis: ambos os autores atuaram no levantamento de requisitos, no projeto, na implementação, no teste e na revisão, em regime de propriedade coletiva do código, no qual qualquer integrante alterava qualquer parte do sistema, com revisão mútua antes da incorporação ao ramo principal \cite{wildt2015}. As demais práticas do método foram adotadas nos termos descritos ao longo deste capítulo: o desenvolvimento guiado por testes, detalhado na Seção~\ref{sec:testes-plataforma}; a integração contínua, acionada a cada mudança submetida ao repositório; o \textit{design} simples e a refatoração contínua, que orientaram a organização em camadas descrita na Seção~\ref{sec:arquitetura-plataforma}.

O restante deste capítulo está organizado da seguinte maneira. A Seção~\ref{sec:arquitetura-plataforma} descreve a modelagem da arquitetura e a divisão em camadas; a Seção~\ref{sec:interpretador-nucleo} situa o núcleo de tradução compartilhado entre os dois volumes; a Seção~\ref{sec:customizacao-dinamica} detalha o mecanismo de customização dinâmica da linguagem, funcionalidade central da plataforma. Em seguida, a Seção~\ref{sec:edicao-codigo} trata da edição de código e do sistema de arquivos virtual; a Seção~\ref{sec:execucao-analises} e a Seção~\ref{sec:terminal-feedback} descrevem, respectivamente, o acionamento das análises a partir da IDE e a execução do código intermediário com retorno ao usuário; e a Seção~\ref{sec:sistema-correcao} apresenta o subsistema de avaliação automatizada. Por fim, a Seção~\ref{sec:acessibilidade-responsividade} discute acessibilidade e responsividade, a Seção~\ref{sec:telas-plataforma} reúne as telas implementadas e a Seção~\ref{sec:testes-plataforma} expõe a estratégia de testes automatizados.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
      x=1cm, y=1cm,
      camada/.style={draw, rounded corners, thick, inner sep=8pt},
      mod/.style={draw, rectangle, align=center, font=\scriptsize,
                  minimum height=0.9cm, minimum width=2.5cm, fill=white},
      rot/.style={draw, rectangle, rounded corners=2pt, align=center, font=\scriptsize,
                  minimum height=0.8cm, fill=white},
      fase/.style={draw, rectangle, align=center, font=\scriptsize,
                   minimum height=0.9cm, minimum width=2.2cm, fill=white},
      titulo/.style={font=\footnotesize\bfseries, anchor=west},
      fl/.style={-{Stealth[length=2.5mm]}, semithick},
      rotulo/.style={font=\tiny, fill=white, inner sep=1.5pt},
    ]

    %% ================= CAMADA 1: apresentacao =================
    \node[mod] (views)   at ( 1.7, 9.0) {\texttt{views}\\composições de tela};
    \node[mod] (comps)   at ( 4.8, 9.0) {\texttt{components}\\\textit{shadcn/ui} e Radix};
    \node[mod] (ctxs)    at ( 7.9, 9.0) {\texttt{contexts}\\Editor, Keyword,\\Terminal, Theme};
    \node[mod] (hooks)   at (11.0, 9.0) {\texttt{hooks}\\useLexerAnalyse,\\useFileSystem};
    \node[mod] (monaco)  at (14.1, 9.0) {Monaco Editor\\dialeto \texttt{java-{}-}};
    \node[mod] (storage) at (17.2, 9.0) {\textit{localStorage}\\arquivos e\\customização ativa};
    \begin{scope}[on background layer]
      \node[camada, fill=blue!4, fit=(views)(storage), inner ysep=17pt] (c1) {};
    \end{scope}
    \node[titulo] at ([shift={(0.15,-0.34)}]c1.north west)
      {Camada de apresentação --- pacote \texttt{ide} (Next.js \textit{Pages Router}, React 19)};

    %% ================= CAMADA 2: nucleo de traducao =================
    \node[fase] (lex)    at ( 2.4, 5.0) {\texttt{Lexer}\\análise léxica};
    \node[fase] (iter)   at ( 6.0, 5.0) {\texttt{TokenIterator}\\\textit{parser} descendente};
    \node[fase] (emit)   at ( 9.6, 5.0) {\texttt{Emitter}\\código de três endereços};
    \node[fase] (interp) at (13.2, 5.0) {\texttt{Interpreter}\\execução};
    \node[fase] (issue)  at (16.8, 5.0) {\texttt{issue} e \texttt{i18n}\\erros e mensagens};
    \draw[fl] (lex) -- (iter);
    \draw[fl] (iter) -- (emit);
    \draw[fl] (emit) -- (interp);
    \begin{scope}[on background layer]
      \node[camada, fill=green!5, fit=(lex)(issue), inner ysep=17pt] (c2) {};
    \end{scope}
    \node[titulo] at ([shift={(0.15,-0.34)}]c2.north west)
      {Núcleo de tradução --- pacote \texttt{compiler} (consumido como dependência local)};

    %% ================= CAMADA 3: rotas locais =================
    \node[rot, minimum width=4.6cm] (apival)    at (3.4, 1.4) {\texttt{/api/submissions/validate}};
    \node[rot, minimum width=4.6cm] (apisearch) at (8.8, 1.4) {\texttt{/api/language-images/search}};
    \begin{scope}[on background layer]
      \node[camada, fill=orange!6, fit=(apival)(apisearch), inner ysep=15pt] (c3) {};
    \end{scope}
    \node[titulo] at ([shift={(0.15,-0.32)}]c3.north west) {Rotas locais do servidor Next.js};

    %% ================= CAMADA 4: persistencia =================
    \node[mod, minimum width=8.4cm] (routers) at (9.5,-2.6)
      {\textit{routers}: \texttt{auth}, \texttt{users}, \texttt{classes}, \texttt{exercises},
       \texttt{exercise-lists}, \texttt{submissions}, \texttt{languages}};
    \node[mod, minimum width=3.9cm] (orm)     at (7.3,-4.2) {SQLAlchemy assíncrono};
    \node[mod, minimum width=3.9cm] (alembic) at (11.7,-4.2) {Alembic (migrações)};
    \begin{scope}[on background layer]
      \node[camada, fill=red!4, fit=(routers)(orm)(alembic), inner ysep=16pt] (c4) {};
    \end{scope}
    \node[titulo] at ([shift={(0.15,-0.32)}]c4.north west)
      {Serviço de persistência --- FastAPI (Python)};

    \node[draw, cylinder, shape border rotate=90, aspect=0.2, minimum width=2.3cm,
          minimum height=1.1cm, align=center, font=\scriptsize] (bd) at (9.5,-6.6) {Banco de\\dados};
    \draw[fl] (c4.south) -- (bd.north);

    %% ================= fluxos entre camadas =================
    \draw[fl] ([xshift=-4cm]c1.south) -- node[rotulo] {\textit{import} direto (\textit{monorepo}), sem rede}
              ([xshift=-4cm]c2.north);
    \draw[fl] (c3.north) -- node[rotulo] {instancia \texttt{Lexer}, \texttt{TokenIterator} e \texttt{Interpreter}} ([xshift=-3.5cm]c2.south);
    \draw[fl] (c3.south) -- node[rotulo] {HTTP} ([xshift=-3cm]c4.north);
    \draw[fl] (c1.east) -- ++(1.1,0) |- node[rotulo, pos=0.28] {\textit{axios} / HTTP (TanStack Query)} (c4.east);
...
Essa estratégia de \textit{feedback} contínuo segue as recomendações de \citeonline{nystrom2021} para ambientes interativos de aprendizagem, reduzindo a distância cognitiva entre a configuração e o resultado observável.
...
A finalização dos fluxos da plataforma foi conduzida com especial atenção aos requisitos de acessibilidade e responsividade. A escolha das bibliotecas \textit{shadcn/ui} e \textit{Radix UI} como base dos componentes primitivos, incluindo diálogos, menus, \textit{tooltips}, alertas, abas e \textit{popovers}, garante, por padrão, o cumprimento das diretrizes WAI-ARIA apresentadas na Seção~\ref{subsec:uxui}, com suporte nativo à navegação por teclado, à leitura por tecnologias assistivas e ao gerenciamento correto de foco. Essa abordagem alinha-se aos princípios de \textit{design} inclusivo discutidos por \citeonline{stefik2013} e \citeonline{ladner2019} na concepção da linguagem Quorum, segundo os quais a acessibilidade não pode ser uma camada adicional, mas deve estar incorporada à fundação dos componentes utilizados.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Aluno/painel_aluno.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Professor/painel_professor.png}
...
A adoção do TDD discutido por \citeonline{aniche2012} foi particularmente útil em construções críticas, como o validador de palavras-chave personalizadas, o construtor de \textit{preview} interativo da customização, o adaptador do grafo de gramática e o \textit{pipeline} de validação de submissões. Nesses pontos, pequenas alterações na lógica poderiam comprometer fluxos inteiros da plataforma, e a presença de testes automatizados garantiu a estabilidade do comportamento mesmo diante de refatorações estruturais frequentes. Em conformidade com a pirâmide de testes apresentada na Seção~\ref{subsec:testes-tdd}, a maior parte da suíte concentra-se no nível unitário, garantindo execuções rápidas e \textit{feedback} imediato, enquanto os testes de integração verificam o comportamento agregado entre componentes, \textit{contexts} e \textit{hooks}.

Para tornar concreta essa estratégia, o Trecho de Código~\ref{lst:teste-storage} reproduz um dos casos que verificam a persistência da customização, construção sobre a qual repousa a continuidade da linguagem escolhida pelo usuário entre recarregamentos da página. O primeiro caso verifica a normalização do nome da linguagem em uma chave de armazenamento; o segundo, que a gravação propaga a linguagem simultaneamente para o registro de linguagens salvas, para a chave de linguagem ativa e para o estado de customização em uso pelo editor.
