...
Este capítulo apresenta a abordagem metodológica adotada para o desenvolvimento da plataforma web proposta neste trabalho, detalhando a arquitetura do Ambiente de Desenvolvimento Integrado (IDE), as estratégias empregadas para a customização dinâmica de \textit{tokens} e sintaxe, os mecanismos de integração com o núcleo do interpretador e os fluxos de execução, avaliação e \textit{feedback} ao usuário. O capítulo registra o estado técnico da plataforma durante a revisão para o TCC~II, a partir da implementação e dos testes disponíveis, contemplando a revisão bibliográfica, a especificação do escopo dos comandos dinâmicos, a modelagem da arquitetura da plataforma, a implementação da customização interativa da linguagem, a execução das análises léxica e sintática a partir do código produzido na IDE, a apresentação dos resultados intermediários no terminal embutido e a finalização dos fluxos principais, com atenção à acessibilidade e à responsividade.

A descrição da implementação foi conferida inicialmente em 16 de setembro de 2026, na revisão \texttt{b4372bf}. Em 19 de setembro, a análise foi atualizada para a revisão \texttt{3ffae01}, incorporando os testes de ponta a ponta do front-end e o fluxo de integração contínua. As verificações de interface e as capturas anteriores conservam suas datas e seu escopo original; os resultados da nova execução de testes são identificados separadamente na Seção~\ref{sec:testes-plataforma} \cite{projeto2026e2e,projeto2026ci,projeto2026frontend}.

O desenvolvimento foi conduzido de forma incremental e iterativa, orientado pelas práticas técnicas do \textit{Extreme Programming} discutidas na Seção~\ref{subsec:xp}. Convém explicitar de que modo essas práticas foram efetivamente instanciadas em uma equipe de dois integrantes, uma vez que o método foi adotado de forma parcial e deliberada. Não se estabeleceram ciclos de duração fixa: o trabalho partiu de um conjunto mínimo de requisitos e avançou por acréscimos funcionais, e sempre que uma melhoria não prevista se mostrava pertinente durante a implementação, ela era incorporada naquele momento, em vez de ser postergada para um planejamento posterior. Essa incorporação contínua da mudança, e não a cadência fixa de entregas, é o traço do método que efetivamente governou o projeto \cite{beck2004,wildt2015}.

Quanto à divisão de trabalho, este TCC concentra a responsabilidade de Igor pelo front-end, enquanto o texto complementar de Victor trata do núcleo de tradução e execução. A colaboração técnica manteve a propriedade coletiva do código: ambos os autores atuaram no levantamento de requisitos, no projeto, na implementação, no teste e na revisão, em regime de propriedade coletiva do código, no qual qualquer integrante alterava qualquer parte do sistema, com alterações registradas no histórico do repositório \cite{wildt2015}. As demais práticas do método foram adotadas nos termos descritos ao longo deste capítulo: o uso de testes automatizados, detalhado na Seção~\ref{sec:testes-plataforma}; o versionamento e a configuração de implantação; o \textit{design} simples e a refatoração contínua, que orientaram a organização em camadas descrita na Seção~\ref{sec:arquitetura-plataforma}.

O restante deste capítulo está organizado da seguinte maneira. A Seção~\ref{sec:arquitetura-plataforma} descreve a modelagem da arquitetura e a divisão em camadas; a Seção~\ref{sec:interpretador-nucleo} situa o núcleo de tradução compartilhado entre os dois textos; a Seção~\ref{sec:customizacao-dinamica} detalha o mecanismo de customização dinâmica da linguagem, funcionalidade central da plataforma. Em seguida, a Seção~\ref{sec:edicao-codigo} trata da edição de código e do sistema de arquivos virtual; a Seção~\ref{sec:execucao-analises} e a Seção~\ref{sec:terminal-feedback} descrevem, respectivamente, o acionamento das análises a partir da IDE e a execução do código intermediário com retorno ao usuário; e a Seção~\ref{sec:sistema-correcao} apresenta o subsistema de avaliação automatizada. Por fim, a Seção~\ref{sec:acessibilidade-responsividade} apresenta a verificação da interface, a Seção~\ref{sec:telas-plataforma} reúne as telas, a Seção~\ref{sec:testes-plataforma} expõe os testes e a Seção~\ref{sec:protocolo-avaliacao} descreve o instrumento de avaliação, cuja análise ficou reservada ao Capítulo~\ref{cap:resultados}.

Em razão da elevada quantidade de telas, componentes interativos e estados compartilhados característicos de uma IDE web, cada fluxo da plataforma, desde a autenticação do usuário até a submissão de exercícios --- o conjunto completo deles está sistematizado nas Figuras~\ref{fig:casos-de-uso} e~\ref{fig:casos-de-uso-pedagogicos} ---, foi tratado como uma unidade autônoma de entrega, implementada e revista ao longo dos incrementos. Essa abordagem foi sustentada por testes automatizados unitários e de integração escritos com o \textit{framework} Vitest, relacionados à discussão de Desenvolvimento Guiado por Testes de \citeonline{aniche2012} e \citeonline{martin_coder}, permitindo a verificação contínua de regressões diante das frequentes mudanças impostas pela natureza altamente interativa da plataforma.

\section{Modelagem da Arquitetura da Plataforma}
\label{sec:arquitetura-plataforma}

A organização da plataforma foi concebida sob a ótica da Separação de Responsabilidades defendida por \citeonline{martin_clean}, dividindo o projeto em três responsabilidades principais: o pacote \texttt{compiler}, responsável pelo núcleo do interpretador; o pacote \texttt{ide}, responsável pela camada de apresentação e por toda a interação com o usuário; e o serviço de \textit{back-end}, responsável pela persistência das entidades de domínio do Ambiente Virtual de Aprendizagem. Os pacotes \texttt{compiler} e \texttt{ide} são versionados em um único repositório no formato \textit{monorepo}, viabilizando o consumo direto do compilador como dependência local da IDE, sem qualquer empacotamento intermediário e sem comunicação remota entre as duas camadas. Esse consumo elimina a requisição remota na análise interativa e permite compartilhar a implementação entre navegador e servidor; não foi medida a redução de latência.

O \textit{front-end} foi implementado com o \textit{framework} Next.js, em sua arquitetura de \textit{Pages Router} e sobre o \textit{Node.js Runtime} padrão --- nenhuma rota declara \texttt{runtime = 'edge'}, de modo que o \textit{Edge Runtime} discutido no referencial teórico não é utilizado neste projeto, opção coerente com a necessidade de acesso pleno às APIs do Node.js pelas rotas de validação de submissões ---, sobre a biblioteca React 19, utilizando TypeScript em todo o código-fonte para verificar tipos durante o desenvolvimento. A estilização foi conduzida com Tailwind CSS em sua quarta versão, complementada por componentes das bibliotecas \textit{shadcn/ui} e \textit{Radix UI}, que fornecem primitivas de semântica e interação, sem garantir a acessibilidade da composição final \cite{radixaccess2026}. O gerenciamento de estado assíncrono foi delegado à biblioteca \textit{TanStack React Query}, responsável pela orquestração de cache, sincronização e revalidação dos dados consumidos das rotas HTTP, enquanto a validação de formulários foi modelada com a biblioteca \textit{Zod}, integrada ao \textit{React Hook Form}.

A camada de \textit{back-end} foi implementada em Python sobre o \textit{framework} FastAPI, escolhido pelas características de alto desempenho, validação automática via \textit{Pydantic} e geração automatizada de documentação OpenAPI discutidas por \citeonline{fastapi2026}. A persistência foi sustentada pelo ORM SQLAlchemy assíncrono, com versionamento de \textit{schema} via Alembic, isolando o modelo de dados das regras de negócio em conformidade com os princípios de Arquitetura Limpa apresentados por \citeonline{martin_clean}. Essa separação mantém o modelo de persistência confinado a uma camada própria, de modo que o núcleo de compilação e a interface web não dependam diretamente do banco de dados nem de suas particularidades. Não se trata de minimizar a importância da persistência, mas de restringir seu alcance: mudanças no esquema ou no mecanismo de armazenamento não devem propagar-se para as regras de tradução nem para a camada de apresentação.

O fluxo geral da Figura~\ref{fig:arquitetura-plataforma} distingue dois ambientes de execução. No navegador, os \textit{hooks} \texttt{useLexerAnalyse} e \texttt{useIntermediatorCode} e o terminal instanciam o núcleo diretamente para analisar, traduzir e executar o programa do editor. No servidor Node.js do Next.js, a rota \texttt{/api/submissions/validate} reutiliza as mesmas classes para avaliar soluções de exercícios. O serviço FastAPI fornece autenticação e recursos persistentes. Após o carregamento da aplicação, os recursos locais não exigem uma chamada a esse serviço para cada análise; isso não equivale a garantir o carregamento inicial da plataforma sem conexão.

\begin{figure}[htbp]
    \centering
    \caption[Arquitetura da plataforma.]{Arquitetura da plataforma: camadas, módulos internos e fluxos de comunicação.}
    \label{fig:arquitetura-plataforma}
    \resizebox{\linewidth}{!}{\input{Figuras/arquitetura-atual.tex}}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}

A organização interna do código da IDE seguiu a topologia clássica de aplicações Next.js, com diretórios dedicados para páginas (\texttt{pages}), componentes reutilizáveis (\texttt{components}), \textit{contexts} de estado global (\texttt{contexts}), \textit{hooks} personalizados (\texttt{hooks}), entidades de domínio (\texttt{entities}), bibliotecas de apoio (\texttt{lib}) e composições de tela (\texttt{views}). Essa segmentação é um primeiro passo na direção da coesão funcional discutida por \citeonline{silveira2012}: a organização em diretórios, isoladamente, não a garante, mas torna explícita a responsabilidade atribuída a cada módulo e facilita que ela seja preservada ao longo da evolução do código, favorecendo a legibilidade e a testabilidade do sistema.

Cabe explicitar quais padrões de projeto foram efetivamente empregados nessa organização. Na camada de apresentação, adota-se o padrão \textit{Provider} da biblioteca React: cada preocupação transversal da aplicação (autenticação, editor, customização da linguagem, terminal, tema, notificações e erros de execução) é isolada em um \textit{context} próprio, sob o diretório \texttt{contexts}, que expõe estado e operações aos componentes descendentes sem exigir a propagação manual de propriedades. Os \textit{hooks} personalizados, por sua vez, operam como fachadas sobre esses \textit{contexts} e sobre o pacote \texttt{compiler}, oferecendo aos componentes uma interface reduzida e estável, independente da forma como o núcleo de tradução é instanciado. A organização interna do núcleo de tradução é detalhada no trabalho complementar; neste texto, o foco recai nos contratos consumidos pelos componentes da IDE.
...
\section{Customização Dinâmica de \textit{Tokens} e Sintaxe}
...
A cada alteração proposta pelo usuário, o componente \texttt{preview-builder} produz, em tempo real, um trecho de código exemplificativo que reflete a customização corrente, oferecendo uma visualização imediata do impacto das mudanças sobre a aparência dos programas. O Trecho de Código~\ref{lst:preview-builder} ilustra a função \texttt{buildDelimiterSnippet}, responsável por interpolar, em um molde fixo, as palavras-chave e os delimitadores efetivamente escolhidos pelo usuário, produzindo um trecho ilustrativo com a configuração corrente. Essa pré-visualização não executa a análise sintática do trecho gerado e, portanto, não comprova que o exemplo seja um programa completo aceito pelo núcleo.
...
A pré-visualização permite inspecionar a escrita resultante antes de salvar a configuração. Um dos itens do instrumento descrito na Seção~\ref{sec:protocolo-avaliacao} pergunta se ela é útil. Saber se ela funciona com iniciantes exigiria o estudo com estudantes registrado ali como etapa seguinte.
...
\subsection{Linguagens predefinidas e identidade da configuração}

A IDE oferece quatro linguagens prontas no seletor, definidas em \texttt{lib/default-languages.ts}: Portugol, Python-like, C-like e Ruby-like. Portugol é a configuração inicial quando não há uma configuração salva aplicável. Essas opções combinam vocabulário e modos já implementados no núcleo; seus nomes indicam inspiração sintática, sem equivalência com as linguagens de propósito geral. A Tabela~\ref{tab:linguagens-predefinidas} resume os modos escolhidos por cada opção.

\begin{table}[htbp]
\centering
\caption{Configurações predefinidas disponíveis no seletor da IDE.}
\label{tab:linguagens-predefinidas}
\footnotesize
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{|p{2.3cm}|p{2.8cm}|p{4cm}|p{4cm}|}
\hline
\textbf{Configuração} & \textbf{Blocos} & \textbf{Tipagem e arranjos} & \textbf{Terminador} \\\hline
Portugol & \texttt{inicio}/\texttt{fim} & Tipos explícitos; arranjos fixos & Ponto e vírgula obrigatório \\\hline
Python-like & Indentação & Sem declaração explícita de tipo; arranjos dinâmicos & Opcional no fim da linha \\\hline
C-like & Chaves & Tipos explícitos; arranjos fixos & Ponto e vírgula obrigatório \\\hline
Ruby-like & \texttt{do}/\texttt{end} & Sem declaração explícita de tipo; arranjos dinâmicos & Opcional no fim da linha \\\hline
\end{tabular}
\legend{Fonte: elaborado pelo autor a partir de \texttt{default-languages.ts}.}
\end{table}

O \textit{hook} \texttt{useLanguageChoices} reúne essas quatro opções com as linguagens locais do visitante ou com o acervo remoto do usuário autenticado. Selecionar uma opção predefinida aplica sua configuração e grava a escolha no armazenamento local, sem criar uma linguagem no banco. Para uma linguagem remota, o \textit{hook} recupera o detalhe e solicita a atualização da linguagem ativa pela API. As predefinições do seletor são distintas dos pontos de partida do assistente, definidos em \texttt{wizard-model.ts}, que incluem também as opções livre e inspirada no vocabulário mineiro.

As palavras canônicas para declarações sem tipo explícito passaram a ser \texttt{variable} e \texttt{function}. Os termos \texttt{variavel} e \texttt{funcao} continuam possíveis como personalizações; a normalização de configurações armazenadas reconhece as chaves antigas e as converte para as atuais, preservando os lexemas escolhidos. O reconhecimento de identificadores e a validação dos nomes personalizados passaram a admitir também \texttt{ç} e \texttt{Ç}, além das letras latinas sem acento e do sublinhado; dígitos são aceitos após o primeiro caractere. Isso não corresponde a aceitar qualquer caractere Unicode.

O assistente mantém nome e descrição nos campos \texttt{name} e \texttt{description}. A edição de uma linguagem existente usa a rota \texttt{/language-creator/[id]}, cujo carregamento inicial ocorre no servidor por \texttt{getServerSideProps}: o identificador é validado, a credencial do \textit{cookie} é encaminhada à API e os dados retornados inicializam o formulário. A criação sem identificador continua disponível sem autenticação, com persistência local. Essa distinção evita tratar a edição remota como uma simples reabertura de um rascunho local.
...
A persistência local dos arquivos do usuário foi modelada pelo \textit{hook} \texttt{useFileSystem}, que implementa um sistema de arquivos virtual sustentado pelo \textit{localStorage} do navegador. Essa abordagem permite que múltiplos arquivos sejam criados, renomeados, removidos e alternados sem qualquer dependência de servidor remoto, permitindo trabalhar com documentos locais após o carregamento da aplicação, sem comprovar funcionamento integral \textit{offline}. O \textit{hook} \texttt{useEditorWithFileSystem} combina o estado do editor com o sistema de arquivos virtual, garantindo a sincronização bidirecional entre o conteúdo exibido no Monaco e o conteúdo persistido localmente. Complementarmente, os \textit{hooks} \texttt{useExplorer} e \texttt{useSearch} oferecem, no painel lateral, navegação por estrutura de arquivos e busca textual sobre todos os arquivos do projeto, replicando as funcionalidades essenciais de uma IDE profissional.

O painel lateral da IDE, implementado em \texttt{SidebarPanel}, reúne as visões de arquivos, busca, linguagem e depuração em uma composição cuja largura pode ser ajustada por arraste. O componente mantém a largura durante sua montagem e aplica um limite mínimo conforme a largura da janela. O ajuste usa eventos de ponteiro; sua presença não demonstra uma operação equivalente por teclado. A revisão também adotou cores derivadas do tema em formulários, painéis e tabelas, cujos resultados de contraste no recorte testado são apresentados na Seção~\ref{sec:acessibilidade-responsividade}.
...
A execução dos programas foi modelada de modo que as operações \texttt{print} e \texttt{scan} produzidas pelo interpretador sejam diretamente conectadas ao terminal embutido. O \texttt{Interpreter}, instanciado no navegador durante a execução interativa e no servidor Next.js durante a validação de submissões, recebe funções de retorno (\texttt{stdout} e \texttt{stdin}) cujo comportamento varia conforme o modo de execução: em modo interativo, as funções são associadas ao terminal da IDE; em modo de validação automatizada de exercícios, são conectadas a \textit{buffers} controlados que capturam a saída para comparação com o resultado esperado. Esse desacoplamento entre o motor de execução e a camada de apresentação atende aos princípios de baixo acoplamento discutidos por \citeonline{martin_clean} e foi inspirado nas abstrações de fluxos de entrada e saída descritas por \citeonline{cooper2012}.

Na depuração, a função \texttt{appendOutputLines} acumula os fragmentos de saída mantendo o estado da linha aberta. Fragmentos sem quebra de linha são concatenados, enquanto as sequências de fim de linha são normalizadas. Assim, a chegada de um novo fragmento durante um passo de execução não cria, por si só, uma linha adicional no terminal.

O núcleo compartilhado também passou a oferecer igualdade estrita (\texttt{===}) e expressão condicional ternária (\texttt{condicao ? valorA : valorB}). Incompatibilidades de atribuição e retorno podem gerar aviso na análise e erro durante a execução. A interface consome esses diagnósticos pelos mesmos canais de avisos e erros usados pelas demais construções. A igualdade estrita e os símbolos do ternário não integram os campos de operadores em forma de palavra do assistente atual.
...
A plataforma incorpora um subsistema de avaliação automatizada relacionado ao ciclo de submissão e veredito documentado pelo Beecrowd \cite{beecrowd2026faq}, materializado pela rota \texttt{/api/submissions/validate}. A modelagem do fluxo de submissão de exercícios partiu da estrutura conceitual desses juízes, mas foi adaptada ao contexto da linguagem dinâmica: enquanto os juízes tradicionais exigem que o aluno se adapte à sintaxe rígida de uma linguagem comercial, a plataforma proposta permite que cada exercício carregue sua própria configuração da linguagem, podendo, inclusive, restringir o aluno a um vocabulário específico definido pelo professor.


Entre as entidades persistidas estão \texttt{User}, \texttt{Class}, \texttt{Exercise}, \texttt{ExerciseList}, \texttt{Submission} e \texttt{Language}. A linguagem armazena a configuração do vocabulário e das variantes gramaticais; não se resume à imagem utilizada como identificação visual. Exercícios e listas podem definir uma política aberta ou vincular uma linguagem obrigatória, e a submissão registra uma cópia da configuração utilizada. O FastAPI e o SQLAlchemy mantêm esses registros, enquanto o \textit{TanStack React Query} gerencia consultas e atualização de dados na interface.
...
\subsection{Autoria de exercícios e acompanhamento de turmas}

A criação de exercícios envia enunciado, política de linguagem e casos de teste na mesma requisição. A edição completa usa \texttt{PUT /exercises/}\allowbreak\texttt{\{id\}}, substituindo também a coleção de casos de teste, que recebe índices de ordenação conforme a sequência enviada. O serviço verifica se o usuário é o professor responsável e valida a política de linguagem. A atualização parcial por \texttt{PATCH} continua disponível para os campos do exercício. Na interface, o diálogo de edição reúne os campos existentes, permite editar, acrescentar e remover casos de teste e atualiza as consultas após a gravação.

A listagem de turmas passou a devolver, além dos dados da turma, um resumo do professor e as contagens de membros e listas publicadas. Os cartões do painel utilizam esse contrato para apresentar o contexto da turma. Esses resumos são calculados a partir dos relacionamentos existentes e não acrescentam tabelas ao modelo de dados.

\subsection{Resolução de exercícios e linguagem obrigatória}

A área de resolução foi decomposta em componentes de cabeçalho, instruções, navegação da lista, painel de submissão e composição da IDE, no diretório \texttt{components/exercise-workspace}. O editor recebe um escopo de armazenamento por exercício e pode iniciar com o código da última submissão disponível. O cabeçalho distingue envio e reenvio, apresenta a nota existente e, quando há uma publicação com prazo vencido, substitui o botão de envio por uma indicação de prazo encerrado. Essa regra de interface não substitui a validação no serviço.

O painel de submissão separa erros de tradução, avisos e resultados dos casos de teste, incluindo quantos passaram e o total executado. A navegação lateral permite alternar entre os itens de uma lista e indicar as submissões disponíveis nesse contexto. Esses recursos dão continuidade ao trabalho no exercício, sem alterar a divisão entre execução local e avaliação no servidor.

Quando o exercício ou a lista exige uma linguagem, a API resolve a precedência entre a restrição do exercício, a da lista e o modo livre. O \texttt{KeywordContext} aplica a configuração recebida como uma sobreposição temporária, com nome, descrição e imagem da linguagem obrigatória. O seletor passa a apresentar apenas essa opção e bloqueia a troca; o estado temporário não é gravado como a preferência pessoal no armazenamento local. Ao encerrar a sobreposição, a configuração anterior é restaurada. O aviso de linguagem obrigatória identifica se a restrição veio do exercício ou da lista.

\section{Acessibilidade, Responsividade e Finalização de Fluxos}
\label{sec:acessibilidade-responsividade}

A acessibilidade foi examinada por inspeção de componentes e por uma verificação técnica delimitada, apresentada a seguir. O módulo \texttt{ui/dialog.tsx} encapsula as primitivas de diálogo, título e descrição do Radix. O componente \texttt{WizardStepper} usa botões nativos e mantém os nomes das etapas com a classe \texttt{sr-only} quando os rótulos deixam de ser visíveis em telas estreitas. Entretanto, a indicação de etapa ativa nesse componente é visual, sem atributo \texttt{aria-current}. Essas evidências mostram recursos e lacunas da implementação; não permitem declarar conformidade global com WAI-ARIA ou WCAG.

A adaptação visual usa utilidades responsivas do Tailwind CSS. No assistente, abaixo do limite \texttt{lg}, a lista de etapas assume disposição horizontal, os botões tornam-se circulares e o cartão da linguagem-base é ocultado. Acima desse limite, as etapas ficam na lateral. O \textit{hook} \texttt{useBreakpoint} também disponibiliza consultas por largura, mas não é o único mecanismo de adaptação da aplicação. Na IDE, a composição usa \texttt{flex-col} e \texttt{sm:flex-row} para reorganizar os painéis. A organização visual deve ser verificada em cada fluxo, pois a existência dessas classes não demonstra, isoladamente, ausência de cortes ou de rolagem indevida.

\input{texto/verificacao-interface}

Os temas claro e escuro são administrados pelo \texttt{ThemeContext}, que aplica a classe \texttt{dark} e persiste a preferência no navegador, com alternância dos temas do Monaco. As Figuras~\ref{fig:home-light} e \ref{fig:home-dark} ilustram essa escolha; a aparência de uma captura não substitui a medição de contraste. As notificações são centralizadas no \texttt{ToastContext}.

\subsection{Internacionalização da interface}
\label{subsec:internacionalizacao}

O requisito RNF03 pede que toda cadeia visível ao usuário esteja externalizada, com suporte a \textit{pt-BR}, \textit{pt-PT}, \textit{es} e \textit{en}. A configuração do Next.js declara esses quatro locais, e os textos ficam no diretório \texttt{i18n/locales}, com um subdiretório por idioma. Cada idioma tem sete arquivos, separados por área da aplicação: \texttt{ui}, \texttt{wizard}, \texttt{toast}, \texttt{ide}, \texttt{footer}, \texttt{language-documentation} e \texttt{language-sample}. Somados, passam de novecentas linhas por idioma.

Os componentes obtêm o texto de duas formas. A maioria chama a função \texttt{t}, exportada por \texttt{i18n}, passando o local corrente e a chave desejada. O assistente de personalização usa o \textit{hook} \texttt{useWizardTranslation}, que lê o local pelo roteador do Next.js e acrescenta o prefixo \texttt{wizard.} às chaves, evitando repetir esse prefixo em cada etapa. As mensagens de erro do compilador seguem caminho próprio, descrito na Seção~\ref{sec:terminal-feedback}: são traduzidas pelo módulo \texttt{i18n} do pacote \texttt{compiler} e adaptadas ao vocabulário da linguagem ativa.

Dois conteúdos didáticos acompanham o idioma escolhido. O primeiro são os arquivos iniciais da IDE, declarados em \texttt{defaultFiles.ts}: a área de trabalho não começa vazia, mas com uma árvore que reproduz a estrutura de um tradutor, em que cada arquivo traz um texto comentado sobre a fase correspondente. Ao trocar de idioma esses arquivos são reescritos, com uma exceção: o arquivo do programa do próprio estudante é preservado, para que a troca não descarte o trabalho já feito. O segundo é a amostra de linguagem exibida no painel lateral, montada pela função \texttt{buildHelloWorldSample} a partir da personalização ativa; os identificadores e as cadeias do exemplo também vêm do pacote de idioma, de modo que um exemplo em português não apresente nomes de variáveis em inglês.

A cobertura, porém, é parcial. Dos 180 componentes da interface, 56 usam o mecanismo de tradução. Estão cobertos o painel do usuário, as telas de turma, a barra de navegação, o terminal, a IDE e o assistente de personalização. Permanecem com cadeias fixas no código as listas de exercícios, o catálogo de exercícios, as submissões, a área de resolução e as telas de linguagens. Um exemplo são os rótulos de envio e de carregamento em \texttt{components/exercise-workspace}, ainda escritos diretamente em português. O requisito RNF03, portanto, não pode ser considerado cumprido.

O \texttt{AuthContext} recupera o perfil autenticado e o utilitário \texttt{auth-cookies} mantém o token em um \textit{cookie} acessível por JavaScript. A interface distingue os papéis administrador, professor, aluno e comunidade, e o modelo de dados inclui ainda um papel interno de sistema. A restrição visual de páginas não substitui as verificações de autorização no serviço. Esta revisão descreve a implementação e não constitui auditoria de segurança.
...
Com o intuito de tornar tangíveis as decisões arquiteturais, os \textit{contexts} de estado, os \textit{hooks} e os componentes discutidos ao longo das seções anteriores, esta seção preserva as capturas das principais telas produzidas nas etapas anteriores. Elas documentam a evolução dos fluxos e não uma reprodução integral da aparência da revisão de 16 de setembro de 2026. As alterações recentes de seleção de linguagem, edição e resolução de exercícios são descritas nas seções anteriores; as capturas da verificação técnica do assistente têm data e escopo próprios. A organização adotada não segue a ordem em que as telas foram construídas, mas as funcionalidades especificadas na Tabela~\ref{tab:requisitos-funcionais}: cada bloco a seguir reúne as telas que materializam um conjunto de requisitos, cujos identificadores são indicados no próprio título. Dessa forma, é possível verificar diretamente quais requisitos já dispõem de interface implementada ao término desta etapa.

\noindent\textbf{Acesso à plataforma e identidade visual (RF01)}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{home_page_light.png}
        \caption{Tema claro.}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{home_page_dark.png}
        \caption{Tema escuro.}
        \label{fig:home-dark}
    \end{subfigure}
...
\noindent\textbf{Consulta de turmas, listas e exercícios pelo aluno (RF15 e RF18)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Aluno/painel_aluno.png}
...
\begin{figure}[htbp]
\centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Aluno/detail_turma.png}
...
\begin{figure}[htbp]
\centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Aluno/Listas/Listas.png}
...
\noindent\textbf{Gestão de turmas e de seus participantes (RF15)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/painel_professor.png}
...
\begin{figure}[htbp]
\centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/detail_turma.png}
...
\begin{figure}[htbp]
\centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/listas_turma.png}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Listas/list.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Listas/config.png}
        \caption{Configuração dos parâmetros gerais.}
        \label{fig:prof-lista-config}
    \end{subfigure}

    \caption{Preparo de uma lista de exercícios pelo professor: listagem das listas existentes e configuração dos parâmetros gerais.}
    \label{fig:professor-listas-preparo}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Listas/add_exec.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Listas/publish.png}
        \caption{Publicação da lista para a turma.}
        \label{fig:prof-lista-publish}
    \end{subfigure}

    \caption{Conclusão da lista de exercícios: vinculação dos enunciados e publicação para a turma.}
    \label{fig:professor-listas-publicacao}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}

\FloatBarrier

A gestão dos enunciados individuais ocorre em um conjunto análogo de telas, ilustrado pelas Figuras~\ref{fig:professor-exercicios-criacao} e \ref{fig:professor-exercicios-acompanhamento}. A tela de listagem permite a busca, a filtragem e a ordenação dos enunciados; a tela de criação centraliza, em um único formulário, a descrição do problema, os casos de teste com entradas e saídas esperadas e a vinculação a uma configuração de linguagem que delimita o vocabulário disponível ao aluno; a tela de detalhamento mostra a composição final do exercício; e a tela de submissões consolida todas as resoluções enviadas pelos alunos, com os respectivos vereditos do subsistema de avaliação automatizada discutido na Seção~\ref{sec:sistema-correcao}.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Exercicios/list.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Exercicios/create.png}
        \caption{Criação de um novo enunciado.}
        \label{fig:prof-exec-create}
    \end{subfigure}

    \caption{Criação de enunciados pelo professor: listagem dos exercícios existentes e formulário de cadastro.}
    \label{fig:professor-exercicios-criacao}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Exercicios/detail.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Professor/Exercicios/execs.png}
        \caption{Submissões dos alunos para o exercício.}
        \label{fig:prof-exec-execs}
    \end{subfigure}

    \caption{Acompanhamento dos enunciados pelo professor: detalhamento do exercício e submissões recebidas.}
    \label{fig:professor-exercicios-acompanhamento}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}

\FloatBarrier

\noindent\textbf{Personalização da linguagem pelo estudante (RF03 a RF06)}

A funcionalidade central da plataforma, que corresponde à personalização dinâmica do vocabulário e das regras estruturais da linguagem, foi materializada em um assistente progressivo cuja versão atual contém sete etapas: identidade, entrada/saída, tipagem, estrutura, operadores, fluxo e revisão. Esse \textit{wizard}, discutido na Seção~\ref{sec:customizacao-dinamica}, conduz o usuário por escolhas independentes, validadas isoladamente, com geração de \textit{preview} em tempo real do código resultante. As capturas a seguir registram a organização anterior em seis etapas, preservada como histórico da implementação; não representam a sequência atual. A Figura~\ref{fig:responsividade-wizard} mostra o assistente na versão verificada nesta revisão.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa1.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa2.png}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa3_1.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa3_2.png}
...
A quarta etapa habilita a substituição de operadores lógicos e relacionais por palavras de uso natural, como uma alternativa de escrita a ser investigada, motivada pela discussão de escolhas sintáticas de \citeonline{stefik2013}. A Figura~\ref{fig:etapas-4} ilustra as duas variações dessa etapa, contemplando, primeiro, a definição de operadores lógicos como \texttt{e}, \texttt{ou} e \texttt{não} e, em seguida, a redefinição dos operadores relacionais por meio do mesmo princípio de palavras significativas.

\begin{figure}[p]
    \centering
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa4_1.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa4_2.png}
...
\begin{figure}[p]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa6_1.png}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa6_2.png}
        \caption{Terminador de instrução.}
        \label{fig:etapa6-2}
    \end{subfigure}

    \caption{Etapa 6 do assistente: definição dos delimitadores de bloco e do terminador de instrução.}
    \label{fig:etapas-6}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{Etapas/etapa6_3.png}
    \caption{Pré-visualização final da linguagem personalizada, exibida ao término do assistente.}
    \label{fig:etapa6-3}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}
...
O conjunto de telas apresentado consolida, sob o ponto de vista visual, as decisões arquiteturais e de modelagem discutidas ao longo deste capítulo. A organização das informações, a coerência entre os perfis de aluno e professor, a granularidade do assistente de personalização e o cuidado com a paridade entre os temas claro e escuro evidenciam o esforço consciente em alinhar a experiência de uso da plataforma aos princípios de UX e de \textit{design} inclusivo discutidos no referencial teórico.

\section{Estratégia de Testes Automatizados}
\label{sec:testes-plataforma}

A validação contínua da plataforma foi sustentada por uma suíte de testes automatizados construída com o \textit{framework} Vitest, complementada por \textit{jsdom} para simulação do ambiente de navegador em testes que envolvem componentes React. A organização dos testes refletiu a topologia interna do projeto: testes unitários acompanham diretamente os módulos correspondentes em diretórios \texttt{\_\_tests\_\_}, enquanto testes de integração foram concentrados em arquivos com sufixos \texttt{.spec.ts} ou \texttt{.spec.tsx} próximos aos componentes que validam \cite{projeto2026frontend}.

Os testes disponíveis verificam construções como configuração da linguagem, persistência, componentes do assistente, depuração e submissões. Eles oferecem evidência do comportamento coberto e apoio à detecção de regressões. A presença de uma suíte, por si só, não comprova que o ciclo teste antes da implementação tenha sido seguido em todos os incrementos. A pirâmide apresentada na Seção~\ref{subsec:testes-tdd} orienta a distinção entre verificações isoladas e integração; não se apresenta aqui uma medição da cobertura total do sistema \cite{vocke2018pyramid,projeto2026frontend}.

A revisão anterior, de 16 de setembro, registrou 139 aprovações em 28 arquivos selecionados pelo comando de testes da IDE, além de seis verificações executadas separadamente. Após os merges, a configuração passou a descobrir arquivos \texttt{.spec.ts} e \texttt{.spec.tsx} em toda a árvore de fontes, mantendo oito exclusões explícitas de testes do assistente ainda pendentes de atualização. Em 19 de setembro de 2026, a execução local do comando \texttt{npm test} do pacote aprovou 274 testes em 54 arquivos. A ampliação documenta o conjunto executado, sem representar a execução dos arquivos excluídos ou uma medição de cobertura total \cite{projeto2026frontend,queiroz2026verificacaotestes}.

A preparação do ambiente de testes passou a incluir uma implementação de armazenamento em memória quando o armazenamento global não fornece as operações esperadas, além de substitutos para APIs de rolagem e consulta de mídia ausentes no ambiente simulado. A nova execução não exigiu o ajuste manual de armazenamento registrado na revisão anterior. Esses substitutos viabilizam os testes dos componentes, mas não reproduzem o cálculo de layout de um navegador real \cite{projeto2026frontend}.

Para tornar concreta essa estratégia, o Trecho de Código~\ref{lst:teste-storage} apresenta um recorte de dois casos que verificam a persistência da customização, construção sobre a qual repousa a continuidade da linguagem escolhida pelo usuário entre recarregamentos da página. Os auxiliares e algumas asserções foram omitidos para abreviar a listagem; o arquivo original contém a verificação completa. O primeiro caso verifica a normalização do nome da linguagem em uma chave de armazenamento; o segundo, que a gravação permite recuperar a linguagem pelo identificador \cite{projeto2026frontend}.

\begin{lstlisting}[language=typescript, caption={Casos de teste da persistência da linguagem personalizada (\texttt{keyword-language-storage.spec.ts}).}, label={lst:teste-storage}, captionpos=b]
describe("keyword-language-storage", () => {
  beforeEach(() => { localStorage.clear(); });

  it("slugifies the language name for localStorage keys", () => {
    expect(slugifyLanguageName("  Minha Linguagem++  "))
      .toBe("minha-linguagem");
    expect(slugifyLanguageName("JAVA   MM")).toBe("java-mm");
  });

  it("persists the saved language in the registry, active key, "
   + "and runtime customization", () => {
    const savedLanguage = createSavedLanguage();

    saveSavedKeywordLanguage(savedLanguage);

    expect(loadSavedKeywordLanguage(savedLanguage.slug))
      .toEqual(savedLanguage);
  });
});
\end{lstlisting}


\input{texto/testes-e2e}

\input{texto/protocolo-avaliacao}
