...
Os temas claro e escuro são administrados pelo \texttt{ThemeContext}, que aplica a classe \texttt{dark} e persiste a preferência no navegador, com alternância dos temas do Monaco. As Figuras~\ref{fig:home-light} e \ref{fig:home-dark} ilustram essa escolha; a aparência de uma captura não substitui a medição de contraste. As notificações são centralizadas no \texttt{ToastContext}. A configuração do Next.js declara os locais \textit{pt-BR}, \textit{pt-PT}, \textit{es} e \textit{en}. Há textos externalizados em \texttt{i18n/locales}, mas também cadeias literais em componentes, como as mensagens de carregamento e envio em \texttt{components/exercise-workspace}; assim, a internacionalização integral permanece um requisito a verificar.
