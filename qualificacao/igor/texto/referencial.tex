...
\subsubsection{Testes de ponta a ponta no front-end}
\label{subsec:fundamentos-e2e}

Os testes de ponta a ponta (E2E) complementam as verificações isoladas ao exercitar jornadas que atravessam diferentes partes da aplicação. Em uma interface web, permitem observar se navegação, componentes, serviços e persistência produzem conjuntamente o resultado esperado. Como essas verificações envolvem mais dependências e tendem a apresentar maior custo de execução e manutenção, sua adoção deve priorizar jornadas relevantes e preservar os testes de menor granularidade. Essa recomendação fundamenta a inclusão de E2E como parte de uma estratégia de regressão, sem pressupor que todos os comportamentos devam ser verificados pela interface \cite{vocke2018pyramid}.

O Playwright Test é uma ferramenta de automação de testes de aplicações web, com executor, asserções e suporte aos navegadores Chromium, Firefox e WebKit. Sua documentação apresenta a execução em diferentes navegadores e ambientes como possibilidades de configuração; usar a ferramenta não implica, por si só, que um projeto tenha exercitado todas essas combinações \cite{playwright2026intro}.

A documentação recomenda verificar comportamentos visíveis ao usuário, manter testes independentes e preferir localizadores baseados em papéis, nomes acessíveis e contratos explícitos de identificação. As asserções que aguardam uma condição, como a visibilidade de uma mensagem, ajudam a sincronizar o teste com a interface. Essas práticas orientam a escrita dos cenários e reduzem a dependência de detalhes incidentais da estrutura visual \cite{playwright2026practices}.

Outra parte da estratégia é executar a suíte no processo de integração contínua, com instalação das dependências e dos navegadores e disponibilização dos relatórios de execução. A documentação do Playwright exemplifica essa configuração no GitHub Actions para eventos de envio de alterações e abertura ou atualização de solicitações de integração. Assim, a verificação passa a ser repetível no fluxo de desenvolvimento, além de permanecer disponível localmente \cite{playwright2026ci}.
...
\subsection{O \textit{Framework} Next.js e sua Arquitetura}

O Next.js é um \textit{framework} baseado em React que oferece roteamento por arquivos e renderização de páginas. No \textit{Pages Router}, adotado neste projeto, os arquivos sob \texttt{pages} definem as páginas, enquanto os arquivos sob \texttt{pages/api} implementam rotas HTTP executadas no servidor. Esse arranjo permite combinar componentes interativos no navegador com operações de servidor em uma mesma aplicação \cite{nextpages2026,nextapi2026}.

As rotas de API são executadas no ambiente Node.js padrão. No projeto, elas atendem, entre outros recursos, à validação de submissões, enquanto a análise e a execução interativa do código do editor ocorrem no navegador. A Seção~\ref{sec:arquitetura-plataforma} mostra essa distribuição. Os conceitos de \textit{App Router} e \textit{Server Actions} não são necessários para descrever a implementação examinada.

\subsection{\textit{Design} de Experiência do Usuário (UX) e Interface do Usuário (UI)}
\label{subsec:uxui}

A experiência do usuário (UX) abrange a interação com um produto e seus serviços; a interface do usuário (UI) corresponde aos elementos pelos quais essa interação acontece. Usabilidade é uma qualidade da interface, relacionada à facilidade de aprendizagem e de uso, e constitui parte da experiência mais ampla \cite{normannielsen1998}.

Neste projeto, essas noções orientam a organização da personalização em etapas, a apresentação de exemplos antes de salvar uma linguagem, a indicação de configurações inválidas e a exposição do estado de execução no terminal. A existência desses recursos pode ser inspecionada no código e na interface. Sua compreensão por iniciantes, entretanto, requer observar tarefas de uso. A Seção~\ref{sec:protocolo-avaliacao} lista as decisões de interface e os itens que perguntam sobre elas. A aparência visual, sozinha, não demonstra usabilidade nem aprendizagem.

\noindent\textbf{Acessibilidade da interface}

A WAI-ARIA define papéis, estados e propriedades para comunicar a semântica de componentes às tecnologias assistivas. Por exemplo, \texttt{role="dialog"} identifica um diálogo; \texttt{aria-expanded} informa o estado de expansão; e \texttt{aria-labelledby} associa um elemento a seu rótulo \cite{w3c2023aria}. Esses atributos não implementam, por si só, os comportamentos de teclado. O guia de práticas ARIA recomenda verificar a interação e a compatibilidade entre navegador e tecnologia assistiva \cite{w3capg2026}.

As WCAG~2.2 fornecem critérios de sucesso que permitem organizar a verificação de acessibilidade do conteúdo web. No recorte deste trabalho, interessam operação por teclado (2.1.1), ausência de bloqueio de teclado (2.1.2), foco visível (2.4.7), contraste mínimo (1.4.3), reorganização do conteúdo (1.4.10), identificação de erros (3.3.1) e nome, papel e valor dos componentes (4.1.2) \cite{w3cwcag22}. A adoção de Radix UI fornece mecanismos de semântica e foco nos componentes primitivos, mas a composição final continua dependendo de rótulos, estilos e interações definidos pela aplicação \cite{radixaccess2026}. A Seção~\ref{sec:acessibilidade-responsividade} delimita a verificação realizada e as lacunas restantes.

\section{Softwares Educativos e Tecnologias de Apoio à Aprendizagem}
\label{sec:softwares-apoio}

Neste trabalho, uma plataforma educativa é entendida como um conjunto de recursos para organizar atividades e apoiar a experimentação de algoritmos. As ferramentas examinadas na seção seguinte concretizam esse apoio de formas distintas: o Portugol Studio expõe a execução do programa \cite{univali2026}, enquanto o Beecrowd oferece submissões confrontadas com casos de teste \cite{beecrowd2026faq}.
...
\textit{Estratégias de interação.} A interação organiza-se em um ciclo curto entre edição, submissão e retorno do resultado. O estudo relatado reúne 17 respostas de alunos e ex-alunos da disciplina de Compiladores do IFCE, Campus Aracati, e apresenta percepções favoráveis sobre o uso da ferramenta. Esses relatos não constituem uma medida controlada de ganho de aprendizagem \cite{ladgelson2021}.
...
\begingroup
\footnotesize
\captionsetup{width=\dimexpr\linewidth-8pt\relax}
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.15}
\begin{longtable}{|>{\raggedright\arraybackslash}p{\dimexpr.14\linewidth-2\tabcolsep\relax}|>{\raggedright\arraybackslash}p{\dimexpr.22\linewidth-2\tabcolsep\relax}|>{\raggedright\arraybackslash}p{\dimexpr.23\linewidth-2\tabcolsep\relax}|>{\raggedright\arraybackslash}p{\dimexpr.23\linewidth-2\tabcolsep\relax}|>{\raggedright\arraybackslash}p{\dimexpr.18\linewidth-2\tabcolsep-6\arrayrulewidth\relax}|}
\caption[Comparativo entre as ferramentas analisadas e a plataforma proposta, segundo os três eixos de análise.]{Comparativo entre as ferramentas analisadas e a plataforma proposta,\\segundo os três eixos de análise.}
...
\endfoot
\multicolumn{5}{l}{Fonte: elaborado pelo autor.}\\
\endlastfoot
        ...
        \hline
        Beecrowd
          & Plataforma web organizada em problemas e submissões
        ...
        \textit{chibicc}
          ...
          & Leitura e reimplementação do código; verificação pela suíte de testes
          & Não \\
        \hline
...
No conjunto comparado, Portugol, Quorum e Beecrowd priorizam a prática em linguagens e ambientes definidos por seus desenvolvedores. Laila permite escrever especificações léxicas e sintáticas, e \textit{chibicc} permite estudar e modificar a implementação do compilador, exigindo conhecimentos próprios desse domínio. A IDE proposta oferece um assistente para configurar alternativas já suportadas pelo núcleo, dispensando a escrita de produções gramaticais ou alterações no código do tradutor.

A hipótese pedagógica é que o assistente favoreça a experimentação com o vocabulário e as convenções da linguagem. O instrumento da Seção~\ref{sec:protocolo-avaliacao} coleta a opinião de especialistas sobre a plausibilidade dessa hipótese. Confirmá-la depende do estudo com estudantes registrado ali como etapa seguinte. O comparativo identifica diferenças funcionais, não superioridade pedagógica da plataforma.
