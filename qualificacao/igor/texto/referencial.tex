...
\noindent\textbf{Execução de Código de Máquina}
...
Para a fundamentação das decisões arquiteturais e dos padrões de projeto adotados neste trabalho, esta subseção baseia-se nos preceitos de Arquitetura Limpa propostos por \citeonline{martin_clean} e nas diretrizes de \textit{design} de software estruturadas por \citeonline{silveira2012}.
...
No contexto da plataforma, a separação entre elementos de interface, regras de customização da linguagem, execução do código e persistência de dados contribui para que cada parte possa evoluir sem comprometer as demais. Essa organização também favorece o uso de padrões de projeto e abstrações voltadas à redução de acoplamento entre os módulos \cite{martin_clean}.
...
\subsubsection{O Framework Next.js e sua Arquitetura}

Para a fundamentação teórica acerca dos conceitos arquiteturais, da evolução do sistema de roteamento e das estratégias de renderização do \textit{framework} Next.js, os parágrafos a seguir foram baseados nas diretrizes e documentações técnicas apresentadas por \citeonline{ziemann2025}, \citeonline{lai2025}, \citeonline{makerkit2026} e \citeonline{vercel2024}.
...
\subsubsection{Design de Experiência do Usuário (UX) e Interface do Usuário (UI)}

Para a fundamentação teórica acerca dos conceitos, diferenças e aplicabilidades do \textit{User Experience} (UX) e \textit{User Interface} (UI), os parágrafos a seguir foram baseados em \citeonline{seyffert2026}, \citeonline{yablonski2020}, \citeonline{lima2024} e \citeonline{raposo2012}.
...
A análise crítica desenvolvida nesta subseção organiza-se em torno das características discutidas anteriormente, tomando como apoio os trabalhos de \citeonline{ladgelson2021}, \citeonline{stefik2013}, \citeonline{ueyama2020}, \citeonline{ferreira2025} e a documentação institucional da \citeonline{univali2026}. A Tabela~\ref{tab:comparativo} sintetiza as principais características das ferramentas examinadas frente à plataforma proposta, evidenciando, em especial, a personalização da sintaxe pelo próprio estudante como traço distintivo da abordagem.

\begin{table}[H]
    \centering
    \caption{Comparativo entre as ferramentas analisadas e a plataforma proposta.}
    \label{tab:comparativo}
    \footnotesize
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|>{\raggedright\arraybackslash}p{2.3cm}|>{\raggedright\arraybackslash}p{1.9cm}|c|>{\raggedright\arraybackslash}p{2.4cm}|>{\raggedright\arraybackslash}p{3.3cm}|c|}
        \hline
        \textbf{Ferramenta} & \textbf{Público-alvo} & \textbf{Web} & \textbf{Gramática} & \textbf{Personalização da sintaxe pelo usuário} & \textbf{Avaliação automatizada} \\
        \hline
        Portugol Studio & Iniciante & Sim & Fixa (PT-BR) & Não & Não \\
        \hline
        Quorum & Iniciante & Sim & Fixa (baseada em evidências) & Não & Não \\
        \hline
        Beecrowd & Iniciante a avançado & Sim & Fixa (linguagens de mercado) & Não & Sim \\
        \hline
        Laila & Avançado & Sim & Definida pelo usuário (gramáticas formais) & Não (foco em compiladores) & Não \\
        \hline
        chibicc & Avançado & Não & Fixa (C11) & Não & Não \\
        \hline
        \textbf{Plataforma proposta} & Iniciante & Sim & Configurável pelo usuário & Sim: palavras-chave, delimitadores de bloco, operadores em forma de palavra, literais booleanos e terminador de instrução & Sim \\
        \hline
    \end{tabular}
