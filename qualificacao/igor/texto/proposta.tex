...
\begin{table}[htbp]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{2.6cm}|>{\raggedright\arraybackslash}p{2.9cm}|>{\raggedright\arraybackslash}p{5.2cm}|}
        ...
        \hline
        Delimitadores de bloco & \texttt{\{} e \texttt{\}} & \texttt{inicio} e \texttt{fim}, ou indentação & No modo delimitado, o par alternativo deve ter identificadores distintos e sem colisões; no modo por indentação, o recuo determina os blocos \\
        \hline
        Terminador de instrução & \texttt{;} & \texttt{ponto} ou terminação por fim de linha & Quando um lexema alternativo for definido, não pode ser vazio, conter espaços, reusar o ponto e vírgula ou caracteres de operadores fixos, nem colidir com palavras já definidas na linguagem \\
        \hline
    \end{tabular}
    \legend{Fonte: elaborado pelo autor, a partir das funções de validação do módulo \texttt{config.ts} do pacote \texttt{compiler}.}
\end{table}

Além das substituições lexicais, a configuração \texttt{GrammarConfig} seleciona alternativas de análise e execução: terminador obrigatório ou opcional por fim de linha; blocos delimitados ou por indentação; modo tipado ou não tipado; e arranjos fixos ou dinâmicos. Essas variantes são combinadas pelo módulo \texttt{compiler-config.ts}; não são novas produções fornecidas pelo usuário. No modo por indentação, delimitadores textuais deixam de ser o mecanismo de delimitação de blocos.



Os fluxos de uso da IDE e de gestão pedagógica especificados neste trabalho, com os respectivos atores, são apresentados nas Figuras~\ref{fig:casos-de-uso} e~\ref{fig:casos-de-uso-pedagogicos}. As duas figuras distinguem quatro atores: o visitante, que pode usar a IDE e a personalização local, além de cadastrar-se e autenticar-se; o usuário autenticado, que acrescenta persistência remota e compartilhamento de linguagens; e os papéis de aluno e de professor, que dele herdam e acrescentam, respectivamente, as funcionalidades de participação em turmas e de gestão pedagógica.

\begin{figure}[p]
\centering
\caption{Casos de uso: acesso e ambiente de desenvolvimento.}
\label{fig:casos-de-uso}
\resizebox{0.95\linewidth}{!}{%
\begin{tikzpicture}[x=1cm,y=1cm,uc/.append style={font=\footnotesize,minimum width=5cm,minimum height=.9cm}]
\draw[thick] (3,1) rectangle (11,-16.0);
\node[font=\small\bfseries] at (7,.65) {Plataforma educacional web};
% Cada ator fica na altura media dos casos a que se liga, para que o leque de
% ligacoes saia equilibrado em vez de descer todo para um lado.
\pic at (0,-7.7) {ator}; \node[font=\small] at (0,-9.35) {Visitante};
\pic at (14,-8.6) {ator}; \node[font=\small,align=center] at (14,-10.4) {Usuário\\autenticado};
\node[uc] (ucCad)  at (7,-0.0)   {Cadastrar-se};
\node[uc] (ucAut)  at (7,-1.5)  {Autenticar-se};
\node[uc] (ucPer)  at (7,-3.0)   {Gerenciar perfil};
\node[uc] (ucLing) at (7,-4.5)  {Criar e gerenciar\\linguagens personalizadas};
\node[uc] (ucCom)  at (7,-6.0)   {Compartilhar e importar\\linguagens da comunidade};
\node[uc] (ucAtiv) at (7,-7.5)  {Ativar linguagem\\no editor};
\node[uc] (ucEdit) at (7,-9.0)   {Editar código na IDE};
\node[uc] (ucArq)  at (7,-10.5)  {Gerenciar arquivos\\do projeto};
\node[uc] (ucExec) at (7,-12.0)  {Executar programa\\no terminal};
\node[uc] (ucTok)  at (7,-13.5) {Inspecionar \textit{tokens} e\\código intermediário};
\node[uc] (ucDep)  at (7,-15.0)  {Depurar execução\\passo a passo};
% As ligacoes partem de alturas ligeiramente diferentes, e nao de um unico
% ponto: junto ao ator elas ficam separadas, em vez de se sobreporem.
\begin{scope}[on background layer]
\foreach \d/\o in {ucCad/0.8,ucAut/0.6,ucLing/0.4,ucAtiv/0.2,ucEdit/0,ucArq/-0.2,ucExec/-0.4,ucTok/-0.6,ucDep/-0.8}
  \draw[liga] (0.35,{-8.1+\o}) -- (\d);
\foreach \d/\o in {ucPer/0.8,ucLing/0.6,ucCom/0.4,ucAtiv/0.2,ucEdit/0,ucArq/-0.2,ucExec/-0.4,ucTok/-0.6,ucDep/-0.8}
  \draw[liga] (13.65,{-9.0+\o}) -- (\d);
\end{scope}
\end{tikzpicture}}
...
\begin{figure}[p]
\centering
\caption{Casos de uso: participação e gestão pedagógica.}
\label{fig:casos-de-uso-pedagogicos}
\resizebox{0.95\linewidth}{!}{%
\begin{tikzpicture}[x=1cm,y=1cm,uc/.append style={font=\footnotesize,minimum width=5cm,minimum height=.9cm}]
\draw[thick] (3,1) rectangle (11,-16.4);
\node[font=\small\bfseries] at (7,.65) {Plataforma educacional web};
\pic at (7,3.2) {ator}; \node[font=\small,align=center] at (7,1.55) {Usuário autenticado};
% Aluno e professor ficam cada um na altura do seu proprio grupo de casos.
% Antes os dois ficavam no topo, e as sete ligacoes do professor cruzavam o
% diagrama inteiro ate a metade de baixo.
\pic at (0,-1.85) {ator}; \node[font=\small] at (0,-3.5) {Aluno};
\pic at (14,-10.4) {ator}; \node[font=\small] at (14,-12.05) {Professor};
\draw[heranca] (0,-1.65) -- (0,2.8) -- (6.55,2.8);
\draw[heranca] (14,-10.2) -- (14,2.8) -- (7.45,2.8);
\node[uc] (ucIng)  at (7,-0.0)   {Ingressar em turma};
\node[uc] (ucCons) at (7,-1.5)  {Consultar listas\\e exercícios};
\node[uc] (ucSub)  at (7,-3.0)   {Submeter solução\\de exercício};
\node[uc] (ucVer)  at (7,-4.5)  {Consultar veredito\\e histórico};
\node[uc] (ucTur)  at (7,-6.4)   {Gerenciar turmas};
\node[uc] (ucMem)  at (7,-7.9)  {Gerenciar membros\\da turma};
\node[uc] (ucEx)   at (7,-9.4)   {Gerenciar exercícios\\e casos de teste};
\node[uc] (ucVinc) at (7,-10.9)  {Vincular linguagem\\a exercício ou lista};
\node[uc] (ucLis)  at (7,-12.4)  {Gerenciar listas\\de exercícios};
\node[uc] (ucPub)  at (7,-13.9) {Publicar lista\\para a turma};
\node[uc] (ucAva)  at (7,-15.4)  {Acompanhar e avaliar\\submissões};
\begin{scope}[on background layer]
\foreach \d/\o in {ucIng/0.45,ucCons/0.15,ucSub/-0.15,ucVer/-0.45}
  \draw[liga] (0.35,{-2.25+\o}) -- (\d);
\foreach \d/\o in {ucTur/0.75,ucMem/0.5,ucEx/0.25,ucVinc/0,ucLis/-0.25,ucPub/-0.5,ucAva/-0.75}
  \draw[liga] (13.65,{-10.8+\o}) -- (\d);
\end{scope}
\end{tikzpicture}}
\legend{Fonte: elaborado pelo autor.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|>{\raggedright\arraybackslash}p{12.3cm}|}
        ...
        \hline
        RNF07 & \textbf{Testabilidade:} as construções críticas devem ser cobertas por testes automatizados, executados localmente e em uma futura etapa de testes do fluxo de integração \\
        \hline
...
\begin{itemize}
    ...
    \item \textbf{Integração, testes e preparação da avaliação:} execução de testes técnicos e elaboração de tarefas, instrumentos e medidas para uma avaliação futura. As atividades de documentação acompanham as etapas técnicas e culminam na redação dos resultados e das limitações.
\end{itemize}

A distribuição temporal dessas atividades ao longo dos doze meses que compreendem o TCC~I e o TCC~II é apresentada na Tabela~\ref{tab:cronograma}, preservando as marcações temporais da pré-proposta e ajustando a descrição da avaliação ao escopo atual. A tabela expressa planejamento, não comprovação de execução. Cada coluna numerada corresponde a um mês corrido das duas etapas e cada célula marcada com ``X'' sinaliza a previsão de execução da respectiva atividade no período indicado.
