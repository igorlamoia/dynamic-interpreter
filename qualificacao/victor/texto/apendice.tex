...
\chapter[APÊNDICE A -- PLANO DE TRABALHO: DIVISÃO DE RESPONSABILIDADES]{\parbox{0.9\textwidth}{\centering APÊNDICE A \textendash{} PLANO DE TRABALHO: DIVISÃO DE RESPONSABILIDADES}} \label{ap:plano}
...
\begin{itemize}
    ...
    \item Análise aprofundada dos parâmetros de configuração da linguagem, que permitem ao núcleo aplicar o vocabulário escolhido e selecionar variantes de escrita predefinidas a cada nova análise e execução;
    ...
    \item Verificação técnica do núcleo de tradução, aferindo o comportamento do interpretador frente a programas de teste elaborados para cobrir as construções da linguagem.
\end{itemize}
...
\begin{itemize}
    ...
    \item Descrição, sob a ótica da interface, dos fluxos de submissão, correção automatizada de exercícios e gerenciamento de turmas;
    \item Definição do protocolo de avaliação da experiência de uso, contemplando os instrumentos de coleta de \textit{feedback}, as métricas de usabilidade e os critérios de engajamento a serem aplicados junto aos estudantes em etapa posterior.
\end{itemize}

\chapter[APÊNDICE B -- DOCUMENTAÇÃO DA API]{\parbox{0.9\textwidth}{\centering APÊNDICE B \textendash{} DOCUMENTAÇÃO DA API}} \label{ap:api}
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{4.3cm}|p{8.3cm}|}
        ...
        \hline
        \texttt{/languages} & Configurações de linguagem, publicação no catálogo e importação. \\
        \hline
...
\chapter[APÊNDICE C -- CASOS DE TESTE AUTOMATIZADOS]{\parbox{0.9\textwidth}{\centering APÊNDICE C \textendash{} CASOS DE TESTE AUTOMATIZADOS}} \label{ap:testes}

Este apêndice resume a suíte de testes automatizados do núcleo do compilador, construída com o \textit{framework} Vitest. A suíte reúne \textbf{224 casos} distribuídos em 21 arquivos de teste, organizados de modo a refletir as camadas internas do projeto, conforme a Tabela~\ref{tab:testes-resumo}. Os casos estão disponíveis no repositório do projeto (\url{https://github.com/igorlamoia/ts-compilator-for-java}), no diretório \texttt{packages/compiler/src/tests}. Na execução de verificação realizada em 19 de setembro de 2026, o comando \texttt{npx vitest run}, a partir do pacote \texttt{compiler}, aprovou os 224 casos, sem falhas. O resultado foi registrado junto à execução do serviço Python \cite{silva2026verificacaobackend}. A distribuição dos testes desse serviço é apresentada na Tabela~\ref{tab:testes-backend}. O total deste apêndice refere-se somente ao compilador e documenta o estado da suíte, sem atribuir a esta execução evidências sobre o que foi demonstrado na apresentação de qualificação.
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{3.4cm}|c|c|p{6.2cm}|}
        ...
        \hline
        Tokens & 1 & 12 & Estabilidade dos identificadores numéricos e dos agrupamentos por categoria. \\
        \hline
        Análise léxica & 9 & 57 & \textit{Scanners} em isolamento: comentários, literais numéricos e de cadeia, terminador configurável, delimitadores de bloco, operadores em forma de palavra e geração de \textit{tokens} de indentação. \\
        \hline
        Análise sintática e semântica & 9 & 142 & Aceitação ou rejeição de programas sob distintos modos de tipagem e de bloco, variantes do \texttt{switch}, ternário, igualdade estrita e verificação de tipos. \\
        \hline
        Interpretação & 2 & 13 & Execução do código de três endereços e sessões de depuração. \\
        \hline
        \textbf{Total} & \textbf{21} & \textbf{224} & \\
        \hline
    \end{tabular}
    \legend{Fonte: elaborado pelo autor.}
\end{table}

\input{texto/dicionario-dados}
