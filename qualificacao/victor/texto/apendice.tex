...
Este apêndice resume a suíte de testes automatizados do núcleo do compilador, construída com o \textit{framework} Vitest. A suíte reúne \textbf{215 casos} distribuídos em 21 arquivos de teste, organizados de modo a refletir as camadas internas do projeto, conforme a Tabela~\ref{tab:testes-resumo}. Os casos estão disponíveis no repositório do projeto (\url{https://github.com/igorlamoia/ts-compilator-for-java}), no diretório \texttt{packages/compiler/src/tests}. Na execução de verificação realizada em 9 de setembro de 2026, o comando \texttt{npm test -- --run}, a partir do pacote \texttt{compiler}, aprovou os 215 casos, sem falhas. Esse resultado documenta o estado atual da suíte, sem atribuir a esta execução evidências sobre o que foi demonstrado na apresentação de qualificação.
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{3.4cm}|c|c|p{6.2cm}|}
        ...
        \hline
        \textbf{Total} & \textbf{21} & \textbf{215} & \\
        \hline
    \end{tabular}
    \legend{Fonte: elaborado pelo autor.}
\end{table}
