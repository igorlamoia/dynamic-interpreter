...
A garantia da qualidade de um software exige a validação contínua de seus comportamentos. Em projetos modernos, a automação de testes complementa a verificação manual e auxilia os desenvolvedores a detectar regressões nos comportamentos cobertos ao alterar, refatorar e evoluir o código. Essa prevenção contra a quebra de comportamentos já consolidados é conhecida como teste de regressão. Além de atestar o funcionamento da aplicação, os testes automatizados atuam como uma documentação viva e executável das regras de negócio do sistema \cite{aniche2012}. %
...
\input{texto/fundamentos-backend}
...
Esta seção compara ferramentas de iniciação em programação e projetos de construção de compiladores com o núcleo desenvolvido neste trabalho. A análise considera três eixos: escolhas de interface, recursos pedagógicos e estratégias de interação com o estudante. A possibilidade de personalização da linguagem pelo usuário é examinada como característica adicional na comparação final.

\subsection{Critérios de Seleção dos Trabalhos}
\label{sec:selecao-trabalhos}

A seleção é intencional e delimitada aos dois eixos do projeto: o uso de linguagens para aprender programação e a construção de seus mecanismos de tradução. Não constitui uma revisão sistemática nem um levantamento exaustivo das ferramentas disponíveis. Foram adotados três critérios de inclusão: (i) finalidade educacional explícita no ensino de programação ou de compiladores; (ii) disponibilidade de documentação, código-fonte ou publicação que permita examinar o funcionamento; e (iii) relação com edição e execução de programas, apresentação da sintaxe ou inspeção das etapas de tradução.

A aplicação desses critérios ao conjunto selecionado é explicitada a seguir:
\begin{itemize}
    \item \textbf{Portugol Studio e Portugol Webstudio:} representam a iniciação por linguagem textual em português e permitem comparar edição, execução e recursos de inspeção de programas. O Studio fornece depuração e inspeção de variáveis; o Webstudio integra a discussão como sua alternativa de uso pelo navegador \cite{univali2026,martins2025}.
    \item \textbf{Quorum:} permite discutir escolhas de sintaxe investigadas experimentalmente e recursos de acessibilidade, aproximando a comparação da hipótese sobre dificuldades sintáticas de iniciantes \cite{stefik2013,quorum2025}.
    \item \textbf{Beecrowd:} representa a prática por exercícios com avaliação automatizada de entrada e saída. Sua inclusão decorre da relação com o fluxo de submissão de exercícios da plataforma, e não de oferecer personalização da sintaxe \cite{ferreira2025,beecrowd2026faq}.
    \item \textbf{Laila:} representa a construção de analisadores léxicos e sintáticos em ambiente web, permitindo comparar a exposição do processo de tradução e a forma de interação com as especificações da linguagem \cite{ladgelson2021}.
    \item \textbf{\textit{chibicc}:} fornece código-fonte e histórico incremental de um compilador C, permitindo comparar a organização das etapas de análise e geração de código com a implementação do núcleo deste trabalho \cite{ueyama2020}.
\end{itemize}

O Scratch, citado na introdução como exemplo de iniciação por blocos, não integra a comparação detalhada porque o recorte escolhido prioriza linguagens textuais, submissão de código e construção de tradutores. Essa delimitação não implica ausência de valor pedagógico ou de extensibilidade no Scratch \cite{scratch2026}. Também ficam fora do recorte IDEs de uso geral sem uma proposta educacional específica e ferramentas para as quais não se dispõe de material suficiente para examinar os três eixos. Não se apresenta uma contagem de exclusões, pois não houve uma busca sistemática com conjunto inicial de candidatos registrado.
...
A comparação evidencia propósitos distintos. Portugol Studio e Quorum oferecem linguagens voltadas à aprendizagem, enquanto Beecrowd organiza a prática por exercícios e vereditos automatizados. Esses recursos respondem a necessidades diferentes: compreender a execução de um programa, escolher uma sintaxe acessível e verificar uma solução contra casos de teste. Sua existência impede tratar a tradução para o português ou a simples execução de exercícios como diferenciais exclusivos da proposta \cite{univali2026,quorum2025,beecrowd2026faq}.

Laila e \textit{chibicc} aproximam-se do eixo de construção de tradutores. A primeira permite elaborar analisadores com FLEX e Bison pelo navegador; o segundo expõe a implementação incremental de um compilador C. O núcleo deste trabalho combina uma representação intermediária observável com um interpretador e com parâmetros de configuração já implementados, sem exigir que o estudante escreva um novo analisador para cada escolha de vocabulário \cite{ladgelson2021,ueyama2020}.

O diferencial técnico proposto é reunir, na mesma IDE, a configuração desses elementos da linguagem, a inspeção dos \textit{tokens} e das instruções intermediárias e a execução do programa. A comparação de funcionalidades não demonstra superioridade pedagógica. Em particular, não permite concluir que a plataforma reduza a carga cognitiva, a reprovação ou o tempo de aprendizagem. Tais efeitos permanecem como hipóteses que exigem avaliação empírica, conforme a delimitação apresentada na introdução.
