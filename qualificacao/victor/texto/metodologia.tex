...
Este capítulo descreve a abordagem metodológica adotada para o desenvolvimento do interpretador dinâmico proposto, detalhando a arquitetura geral do sistema, as decisões de projeto que sustentam cada fase do compilador implementado e os mecanismos de integração com a plataforma web. A condução do trabalho seguiu as etapas previstas no cronograma do projeto, contemplando, na revisão para a segunda fase do Trabalho de Conclusão de Curso, a revisão bibliográfica, a especificação do escopo da linguagem e de seus elementos configuráveis, a modelagem da arquitetura do interpretador, a implementação das análises léxica e sintática, a geração e a execução do código intermediário e a integração entre o núcleo do compilador e o ambiente web.

A descrição da implementação foi conferida inicialmente em 16 de setembro de 2026, na revisão \texttt{b4372bf}. Em 19 de setembro, a análise foi ampliada para a revisão \texttt{e0a012b}, contemplando os testes do serviço Python, os contratos de resposta, os controles de acesso e o fluxo de integração contínua. Os resultados locais da nova execução são identificados nas seções de testes; as capturas anteriores conservam seu caráter histórico \cite{projeto2026backend,projeto2026backendtestes,projeto2026workflow,silva2026verificacaobackend}.
...
Quanto à divisão de trabalho, este TCC concentra a atuação de Victor no núcleo de tradução e no back-end, enquanto o texto complementar de Igor trata do front-end. A colaboração manteve a propriedade coletiva do código: ambos os autores atuaram no levantamento de requisitos, no projeto, na implementação, no teste e na revisão, podendo alterar diferentes partes do sistema, com alterações registradas no histórico do repositório \cite{wildt2015}. As demais práticas foram adotadas nos termos descritos ao longo deste capítulo: o uso de testes automatizados, detalhado na Seção~\ref{sec:metodologia-testes}; o versionamento e o fluxo de implantação; o \textit{design} simples e a refatoração contínua, que orientaram a organização descrita na Seção~\ref{sec:modelagem-arquitetura}.

O restante deste capítulo está organizado da seguinte maneira. A Seção~\ref{sec:modelagem-arquitetura} descreve a modelagem da arquitetura do projeto; a Seção~\ref{sec:especificacao-linguagem} especifica a linguagem e seus elementos configuráveis; a Seção~\ref{sec:metodologia-interpretador} detalha o núcleo do interpretador, fase a fase; a Seção~\ref{sec:integracao-plataforma} trata da integração entre o núcleo e a plataforma web; a Seção~\ref{sec:contratos-seguranca-backend} apresenta os contratos de resposta e os controles do back-end; e a Seção~\ref{sec:metodologia-testes} expõe a estratégia de testes automatizados.

No ciclo de desenvolvimento guiado por testes tomado como referência, a escrita do teste precede a implementação da funcionalidade: primeiro se especifica o comportamento esperado em um teste que falha, depois se implementa o código necessário para fazê-lo passar e, por fim, se refatora preservando a aprovação da suíte. A integração de uma etapa à seguinte é uma verificação adicional, não o que caracteriza o TDD, conforme a prática consolidada por \citeonline{aniche2012} e \citeonline{martin_coder}. Os testes permitem verificar regressões nas configurações da linguagem; sua presença no repositório não comprova, isoladamente, a ordem teste--implementação--refatoração em cada incremento.
...
\begin{figure}[p]
    ...
    \input{Figuras/arquitetura-atual.tex}
...
O conteúdo pedagógico é distribuído entre \texttt{exercises}, \texttt{test\_cases}, \texttt{exercise\_lists} e \texttt{exercise\_list\_items}. Exercícios e listas possuem política de linguagem \texttt{OPEN} ou \texttt{LOCKED} e uma referência opcional em \texttt{locked\_language\_id}. Uma restrição de verificação exige que essa referência esteja preenchida exatamente quando a política é \texttt{LOCKED}. Os casos de teste pertencem a exercícios e armazenam entrada e saída esperada; os itens de lista associam exercícios a listas, com peso e ordem. O relacionamento de exercícios com casos de teste ordena os resultados por \texttt{order\_index}. A remoção de casos de teste dependentes está configurada no relacionamento do ORM por \texttt{cascade="all, delete-orphan"}, o que deve ser distinguido de uma cláusula SQL \texttt{ON DELETE CASCADE}.
...
Na versão examinada, \texttt{POST /exercises} recebe também os casos de teste, e \texttt{PUT /exercises/}\allowbreak\texttt{\{id\}} substitui os dados do exercício e sua coleção ordenada de testes. A atualização parcial por \texttt{PATCH} permanece distinta da substituição completa. A listagem \texttt{GET /classes} retorna dados resumidos do professor e contagens de membros e listas. Esses contratos apoiam os fluxos da IDE sem alterar o número de tabelas e atributos do esquema.
...
A linguagem suportada pelo interpretador foi projetada como uma linguagem imperativa didática, com variantes de tipagem configuráveis, suficientemente expressiva para abranger os conteúdos típicos das disciplinas introdutórias de programação, e simultaneamente flexível para que professores e alunos possam customizar suas regras superficiais. O conjunto base de funcionalidades contempla declarações com ou sem tipo explícito, atribuições, expressões aritméticas, lógicas, relacionais e condicionais ternárias, estruturas condicionais (\texttt{if}/\texttt{else} e \texttt{switch}), estruturas de repetição (\texttt{for} e \texttt{while}), arranjos uni e multidimensionais com modos de alocação fixos e dinâmicos, e operações de entrada e saída padrão (\texttt{print} e \texttt{scan}).
...
No modo sem declaração explícita de tipo, as palavras canônicas são \texttt{variable} e \texttt{function}; \texttt{variavel} e \texttt{funcao} passam a ser exemplos de personalização. A IDE migra as chaves antigas das configurações armazenadas para os nomes atuais. O léxico admite \texttt{ç} e \texttt{Ç} em identificadores e nomes configuráveis, além das letras latinas sem acento e do sublinhado, com dígitos permitidos após o primeiro caractere. Esse conjunto não representa suporte irrestrito a Unicode.

A IDE disponibiliza quatro configurações prontas: Portugol, Python-like, C-like e Ruby-like. Portugol, a opção inicial na ausência de configuração salva aplicável, utiliza tipos explícitos, arranjos fixos, blocos \texttt{inicio}/\texttt{fim} e ponto e vírgula obrigatório. C-like combina os mesmos modos com chaves; Python-like combina indentação, declarações sem tipo explícito, arranjos dinâmicos e terminador opcional no fim da linha; Ruby-like utiliza os três últimos modos com blocos \texttt{do}/\texttt{end}. Esses nomes indicam combinações suportadas pelo núcleo, sem compatibilidade integral com Python, C ou Ruby. Os padrões da IDE também não devem ser confundidos com os valores padrão de \texttt{GrammarConfig} quando o núcleo é instanciado diretamente.
...
A função \texttt{exprStmt} reconhece a expressão condicional \texttt{condicao ? valorA : valorB} após as expressões lógicas de disjunção, conferindo-lhe menor precedência. As chamadas recursivas permitem condicionais aninhadas. A tradução emite rótulos, um salto condicional e atribuições a uma variável temporária, de modo que apenas o ramo selecionado seja executado. Ramos de mesmo tipo preservam esse tipo; a combinação de inteiro e real resulta em real, a presença de tipo dinâmico resulta em dinâmico e as demais combinações são classificadas como desconhecidas.
...
A verificação de conversão diferencia avisos de erros de execução. Converter \texttt{float} para \texttt{int} gera um aviso de possível perda; promover \texttt{int} para \texttt{float} é permitido. Outras incompatibilidades conhecidas geram o aviso \texttt{grammar.\allowbreak{}incompatible\_\allowbreak{}type\_\allowbreak{}conversion}; os tipos dinâmico e desconhecido adiam essa verificação. Na execução, \texttt{coerceCheckedValue} rejeita, por exemplo, atribuir uma cadeia a uma variável inteira ou retornar um booleano de uma função inteira, emitindo os códigos \texttt{interpreter.\allowbreak{}incompatible\_\allowbreak{}assignment} ou \texttt{interpreter.\allowbreak{}incompatible\_\allowbreak{}return}. Assim, nem toda incompatibilidade interrompe a tradução: alguns casos são avisados antes e rejeitados ao executar a instrução.

\subsection{Geração de Código Intermediário}
\label{subsec:geracao-codigo-intermediario}

A representação intermediária adotada é uma variante de código de três endereços, conforme caracterizado por \citeonline{aho1995} e \citeonline{cooper2012}, materializada pela interface \texttt{Instruction}, contendo um operador \texttt{op}, um destino \texttt{result} e dois operandos \texttt{operand1} e \texttt{operand2}. O conjunto de operadores suportados é definido como uma união de tipos em \texttt{interpreter/constants.ts} e inclui as seis operações aritméticas (\texttt{+}, \texttt{-}, \texttt{*}, \texttt{/}, \texttt{\%} e \texttt{//}), as três operações lógicas (\texttt{||}, \texttt{\&\&} e \texttt{!}), as sete operações relacionais, incluindo a igualdade estrita (\texttt{===}), atribuições, declarações simples e de \textit{arrays}, instruções de acesso e atualização de \textit{arrays} (\texttt{ARRAY\_GET} e \texttt{ARRAY\_SET}), saltos condicionais e incondicionais (\texttt{IF}, \texttt{JUMP} e \texttt{RETURN}), rótulos (\texttt{LABEL}) e chamadas de sistema (\texttt{CALL}).
...
A inclusão do ternário reutiliza essas instruções de controle de fluxo, sem criar uma instrução intermediária exclusiva para a construção. Já a igualdade estrita possui seu próprio operador \texttt{===} e o identificador de \textit{token} 64; o ponto de interrogação usa o identificador 65. Os identificadores anteriores de palavras-chave foram preservados durante a mudança de \texttt{variavel}/\texttt{funcao} para \texttt{variable}/\texttt{function}.
...
A igualdade comum (\texttt{==}) compara valores com coerção, enquanto \texttt{===} exige também compatibilidade dos tipos identificados em execução. A implementação consulta o tipo declarado quando disponível e classifica o valor nos demais casos. Nos testes, uma cadeia \texttt{"1"} e um inteiro \texttt{1} são iguais por \texttt{==}, mas diferentes por \texttt{===}; uma variável \texttt{float} com valor \texttt{1.0} também difere estritamente de uma variável \texttt{int} com valor \texttt{1}. O operador \texttt{!=}, traduzido internamente como \texttt{<>}, mantém a comparação de desigualdade estrita entre valores do hospedeiro; portanto, não é sempre o complemento de \texttt{==} quando os tipos diferem. A igualdade estrita e os símbolos do ternário não possuem campos de substituição por palavras em \texttt{OperatorWordMap}.
...
Nos exercícios com linguagem obrigatória, a interface aplica temporariamente a configuração efetiva resolvida pela API, com precedência do exercício sobre a lista e sobre a seleção livre. A sobreposição bloqueia a seleção de outra linguagem e preserva a configuração pessoal para restauração. O servidor de validação continua recebendo o código e a configuração pelo contrato de submissão; o bloqueio do seletor é um comportamento da interface, não uma demonstração de validação de todas as restrições no servidor.

\input{texto/contratos-seguranca-backend}

\section{Estratégia de Testes Automatizados}
\label{sec:metodologia-testes}

\subsection{Testes do núcleo de tradução}
...
Na verificação local de 19 de setembro de 2026, a suíte do núcleo aprovou novamente 224 testes em 21 arquivos, mantendo o total registrado em 16 de setembro \cite{silva2026verificacaobackend}. Os casos incluem igualdade comum e estrita, ternários aninhados com execução somente do ramo selecionado, avisos de conversão e rejeição de atribuições e retornos incompatíveis. O Apêndice~\ref{ap:testes} apresenta a distribuição e o comando utilizado. Os resultados documentam os comportamentos cobertos, sem constituir prova de correção de todos os programas aceitos.

\input{texto/testes-backend}
