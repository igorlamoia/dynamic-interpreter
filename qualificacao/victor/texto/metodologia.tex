...
A construção do sistema foi conduzida de forma incremental e iterativa, organizando o trabalho em ciclos curtos de implementação e validação. Não se adotou formalmente um \textit{framework} de gestão ágil (cujos papéis e cerimônias pressupõem equipes maiores do que a dupla responsável por este projeto), mas sim os princípios de desenvolvimento iterativo e incremental discutidos por \citeonline{agile2012}, combinados à prática de Desenvolvimento Guiado por Testes consolidada por \citeonline{aniche2012} e \citeonline{martin_coder}. Cada etapa do compilador foi modelada, implementada e validada por meio de testes automatizados antes da agregação à etapa seguinte. Essa abordagem permitiu a verificação contínua de regressões durante a evolução das gramáticas configuráveis e a manutenção da estabilidade das fases de análise mesmo diante das frequentes mudanças impostas pela natureza dinâmica da linguagem proposta.
