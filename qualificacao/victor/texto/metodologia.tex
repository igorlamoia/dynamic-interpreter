...
O restante deste capítulo está organizado da seguinte maneira. A Seção~\ref{sec:selecao-trabalhos} explicita o critério de seleção dos trabalhos relacionados; a Seção~\ref{sec:modelagem-arquitetura} descreve a modelagem da arquitetura do projeto; a Seção~\ref{sec:especificacao-linguagem} especifica a linguagem e seus elementos configuráveis; a Seção~\ref{sec:metodologia-interpretador} detalha o núcleo do interpretador, fase a fase; a Seção~\ref{sec:integracao-plataforma} trata da integração entre o núcleo e a plataforma web; e a Seção~\ref{sec:metodologia-testes} expõe a estratégia de testes automatizados.

Cada etapa do compilador foi modelada, implementada e validada por meio de testes automatizados antes da agregação à etapa seguinte, conforme a prática de Desenvolvimento Guiado por Testes consolidada por \citeonline{aniche2012} e \citeonline{martin_coder}. Essa abordagem permitiu a verificação contínua de regressões durante a evolução das gramáticas configuráveis e a manutenção da estabilidade das fases de análise mesmo diante das frequentes mudanças impostas pela natureza dinâmica da linguagem proposta.

\section{Seleção dos Trabalhos Relacionados}
\label{sec:selecao-trabalhos}

A escolha dos trabalhos analisados na Seção~\ref{sec:trabalhos-relacionados} não seguiu uma revisão sistemática da literatura, mas um critério de representatividade: foram selecionadas ferramentas e linguagens que ilustram os dois eixos por meio dos quais a literatura aborda o problema tratado neste trabalho. O primeiro eixo reúne soluções voltadas ao apoio à lógica e à prática de programação para iniciantes, com ênfase na redução da barreira sintática, e é representado pelo Portugol Studio, pela linguagem Quorum e pela plataforma Beecrowd. O segundo eixo reúne ferramentas voltadas ao ensino da construção de compiladores e analisadores, e é representado pela plataforma Laila e pelo projeto \textit{chibicc}.

Como critérios de inclusão, considerou-se que cada trabalho (i) tivesse propósito educacional explícito no ensino de programação ou de compiladores; (ii) fosse acessível para análise, por meio de documentação, publicação acadêmica ou ferramenta disponível publicamente; e (iii) guardasse relação direta com ao menos uma das dimensões da solução proposta (a personalização da sintaxe, a execução interativa ou a visualização do processo de tradução). Cada ferramenta foi então examinada quanto à sua proposta, às suas características e às suas limitações, culminando na análise crítica que evidencia o diferencial da plataforma desenvolvida.
...
A camada de persistência da plataforma foi projetada em conformidade com os princípios de normalização relacional discutidos por \citeonline{elmasri2015}, adotando o gerenciador de banco de dados PostgreSQL e o esquema lógico ilustrado na Figura \ref{fig:esquema-banco}. As entidades foram separadas para reduzir redundâncias, preservar dependências funcionais e manter a integridade referencial entre instituições, usuários, turmas, exercícios, listas e submissões. O modelo organiza as entidades em torno de três eixos principais: a gestão institucional, representada pelas entidades \texttt{organizations}, \texttt{users} e \texttt{classes}; a gestão de conteúdo pedagógico, compreendendo \texttt{exercises}, \texttt{exercise\_lists} e \texttt{test\_cases}; e o registro de atividades dos estudantes, materializado pelas entidades \texttt{submissions} e pelas tabelas de associação \texttt{class\_members}, \texttt{exercise\_list\_items} e \texttt{class\_exercise\_lists}.

\begin{figure}[H]
    \centering
    \caption{Esquema lógico do banco de dados da plataforma.}
    \label{fig:esquema-banco}
    \includegraphics[width=0.95\textwidth]{Figuras/esquema_banco.png}
    \legend{Fonte: elaborado pelo autor.}
\end{figure}

A entidade \texttt{organizations} representa as instituições de ensino cadastradas na plataforma, agrupando professores e alunos sob um contexto administrativo comum. A entidade \texttt{users} unifica os dois perfis de usuário da plataforma, professor e estudante, por meio de um atributo de papel (\texttt{role}), cujos valores possíveis são definidos pelo tipo enumerado \texttt{userrole}, mantendo uma única tabela de autenticação e evitando a duplicação de atributos compartilhados como nome, endereço de correio eletrônico e imagem de perfil. Cada usuário é obrigatoriamente associado a uma organização, garantindo o isolamento multilocatário dos dados.

A entidade \texttt{classes} representa as turmas criadas por professores e é identificada externamente por um código de acesso único (\texttt{access\_code}), utilizado pelos estudantes para se matricularem. O ciclo de vida de uma turma é controlado pelo tipo enumerado \texttt{classstatus}, que distingue turmas ativas, encerradas ou arquivadas. A relação de matrícula é mantida pela tabela associativa \texttt{class\_members}, que registra o estudante e o instante de ingresso na turma.

Os exercícios individuais são cadastrados na entidade \texttt{exercises}, vinculados ao professor responsável pela sua criação. Cada exercício pode possuir múltiplos casos de teste, armazenados em \texttt{test\_cases}, que registram a entrada fornecida ao programa e a saída esperada para fins de avaliação automática, conforme o ciclo de validação descrito na Seção \ref{sec:integracao-plataforma}. A relação de dependência entre exercício e casos de teste é implementada com exclusão em cascata, de modo que a remoção de um exercício elimine automaticamente seus respectivos casos de teste, preservando a integridade referencial do esquema.

As listas de exercícios são modeladas pela entidade \texttt{exercise\_lists}, criadas pelo professor e compostas por itens representados pela tabela \texttt{exercise\_list\_items}. Esta tabela associativa, além de referenciar o exercício e a lista, armazena o peso da nota (\texttt{grade\_weight}) e o índice de ordenação (\texttt{order\_index}), permitindo que o professor controle tanto a distribuição de pontos quanto a sequência de apresentação dos itens aos estudantes. A publicação de uma lista para uma turma específica é gerenciada pela tabela \texttt{class\_exercise\_lists}, que registra o prazo de entrega (\texttt{deadline}), a nota total (\texttt{total\_grade}), o número mínimo de exercícios exigidos (\texttt{min\_required}) e os instantes de publicação e de última atualização, conferindo ao professor controle granular sobre a disponibilização do conteúdo.

A entidade \texttt{submissions} registra cada tentativa de envio realizada por um estudante, capturando um instantâneo imutável do código submetido (\texttt{code\_snapshot}), o resultado da avaliação automática expresso pelo tipo enumerado \texttt{submissionstatus} e a pontuação calculada com base nos casos de teste aprovados. O campo de retroalimentação do professor (\texttt{teacher\_feedback}) permite a complementação qualitativa da avaliação automática. A referência simultânea ao exercício, à lista de exercícios e à turma possibilita consultas agregadas de desempenho tanto por estudante quanto por turma ou por lista, sem exigir junções adicionais. Índices secundários foram definidos sobre os campos \texttt{(exercise\_list\_id, class\_id)} e \texttt{student\_id} para otimizar as consultas de maior frequência, como a listagem de submissões por turma e a visualização do histórico de um estudante, em conformidade com as diretrizes de desempenho discutidas por \citeonline{elmasri2015}.

O versionamento do esquema lógico é gerenciado pelo Alembic, cujo estado corrente de migração é rastreado pela tabela \texttt{alembic\_version}. Essa abordagem permite a evolução incremental do banco de dados sem perda de dados em implantações sucessivas, em consonância com a estratégia de entrega contínua adotada no projeto.
...
\begin{itemize}
    ...
    \item \textbf{Usuários} (\texttt{/users}): expõe operações de consulta e atualização de perfil para os dois papéis suportados, professor e estudante, com controle de acesso baseado no atributo \texttt{role};
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figuras/validateOperatorWordMap.png}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figuras/validateBooleanLiteralMap.png}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figuras/validateBlockDelimiter.png}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figuras/validateStatementTerminatorLexeme.png}
...
Para tornar concreta essa estratégia, o Trecho de Código~\ref{lst:teste-aliases} reproduz um dos casos que verificam a injeção dinâmica de regras gramaticais no analisador léxico. O teste instancia o \texttt{Lexer} com um mapeamento que substitui os operadores lógicos por palavras e verifica que os \textit{tokens} produzidos carregam exatamente os mesmos identificadores de tipo que seriam gerados pelos símbolos canônicos. Essa equivalência é o que garante que as fases seguintes do compilador permaneçam indiferentes à personalização escolhida pelo usuário.
