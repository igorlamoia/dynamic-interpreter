...
Na verificação local de 16 de setembro de 2026, a suíte do núcleo aprovou 224 testes em 21 arquivos. Os casos incluem igualdade comum e estrita, ternários aninhados com execução somente do ramo selecionado, avisos de conversão e rejeição de atribuições e retornos incompatíveis. O Apêndice~\ref{ap:testes} apresenta a distribuição e o comando utilizado. Os resultados documentam os comportamentos cobertos, sem constituir prova de correção de todos os programas aceitos.

O fluxo de implantação versionado é acionado por alterações no ramo \texttt{main} e executa um \textit{script} externo por SSH. Ele não declara uma etapa Vitest, e o conteúdo desse \textit{script} não está no conjunto examinado. Portanto, esta revisão registra execução local dos testes e não pressupõe sua execução automática como condição de integração.
