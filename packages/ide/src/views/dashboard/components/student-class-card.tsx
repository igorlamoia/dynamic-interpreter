...
export function JoinClassCard({ onJoinClick }: { onJoinClick: () => void }) {
  return (
    <button onClick={onJoinClick} className="rounded-xl border-2 border-dashed border-border dark:border-white/10 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all p-6 flex flex-col items-center justify-center group h-full min-h-[300px]">
      ...
      <p className="text-muted-foreground text-sm hidden sm:block text-center px-4">Insira o código fornecido pelo seu professor para participar.</p>
    </button>
  );
}
